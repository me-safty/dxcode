...
Generated from local `main` at `2492d3d25` and merged upstream `main` at `e95b57dc2` (`v0.0.28-nightly.20260618.594`) in merge commit `a4bd3b049`. Before the merge, this branch was 207 commits ahead and 1 commit behind the merged upstream ref; after the merge and follow-up customization commits through `f60c5e6ae`, this branch is 210 commits ahead and 0 commits behind the merged upstream ref. The local `upstream/main` ref has since advanced to `20f37f367`, making the branch 210 commits ahead and 6 commits behind current `upstream/main`. The incoming upstream diff from the merge base was 606 files changed, 37206 insertions, and 53661 deletions; the current fork delta against the merged upstream ref is 241 files changed, 32974 insertions, and 751 deletions.
...
- The local Version Control panel now calls the existing `vcs.panel.*` RPCs through `vcsEnvironment` atom commands and keeps panel expansion/selection/enrichment logic in `apps/web/src/state/sourceControlPanel.ts`, while preserving its right-panel surface and live VCS status integration.
...
- Shared active-project/thread scoping for subagents and source-control state now lives in `packages/client-runtime/src/environment/workspaceScope.ts`, so client surfaces do not reimplement the same workspace filtering after upstream's connection-runtime rewrite.
