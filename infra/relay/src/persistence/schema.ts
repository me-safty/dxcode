...
export const relayMobileDevices = pgTable(
  ...
  (table) => [
    ...
    uniqueIndex("idx_relay_mobile_devices_expo_push_token").on(table.expoPushToken),
...
export const relayDeliveryAttempts = pgTable(
  "relay_delivery_attempts",
  {
    ...
    deliveryProvider: varchar("delivery_provider", { length: 16 }).$type<"apns" | "expo">(),
  ...
  (table) => [
    ...
    index("idx_relay_delivery_attempts_expo_receipts").on(
