...
function resolveRecentThreadAgent(): AgentChoice {
  const [latestShell] = selectThreadShellsAcrossEnvironments(useStore.getState()).sort(
    (left, right) =>
...
export function resolveRecentAgent(
  ...
  return pickRecentAgent({
    ...
    recentThread: resolveRecentThreadAgent(),
