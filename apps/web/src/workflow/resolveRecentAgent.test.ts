...
const makeEnvironmentState = (shells: ReadonlyArray<ThreadShell>): EnvironmentState => ({
  projectIds: [projectId],
  projectById: {
    [projectId]: {
      id: projectId,
      environmentId,
      name: "Recent agent project",
      cwd: "/tmp/recent-agent",
  ...
  threadIdsByProjectId: { [projectId]: shells.map((shell) => shell.id) },
...
describe("resolveRecentAgent", () => {
  it("uses thread shells for the most recent available agent before defaulting", () => {
    ...
    useStore.setState({
      ...
      environmentStateById: {
        [environmentId]: makeEnvironmentState([olderShell, newerShell]),
      },
    });

    expect(resolveRecentAgent()).toEqual({ instance: "claude_main", model: "sonnet" });
  });
});
