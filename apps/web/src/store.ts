...
export function syncServerReadModel(state: AppState, readModel: OrchestrationReadModel): AppState {
  ...
  return {
    ...
    sidebarThreadsById,
    threadIdsByProjectId,
    latestAppliedOrchestrationSequence: Math.max(
      state.latestAppliedOrchestrationSequence,
...
export function applyOrchestrationEvent(state: AppState, event: OrchestrationEvent): AppState {
  ...
  const nextLatestAppliedOrchestrationSequence = Math.max(
    nextState.latestAppliedOrchestrationSequence,
    event.sequence,
  );
  return nextState.latestAppliedOrchestrationSequence === nextLatestAppliedOrchestrationSequence
    ? nextState
    : {
        ...nextState,
        latestAppliedOrchestrationSequence: nextLatestAppliedOrchestrationSequence,
      };
}
