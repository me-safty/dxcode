...
import { chatPinId, usePinnedItemsStore } from "../pinnedItemsStore";
...
const SidebarProjectItem = memo(function SidebarProjectItem(props: SidebarProjectItemProps) {
  ...
  const handleOpenEditorClick = useCallback(
    ...
    [isMobile, project.memberProjects, router, setOpenMobile],
  );
  ...
  const handleThreadContextMenu = useCallback(
    async (threadRef: ScopedThreadRef, position: { x: number; y: number }) => {
      ...
      const pinId = chatPinId(threadRef.environmentId, threadRef.threadId);
      const chatPinned = usePinnedItemsStore.getState().isPinned(pinId);
      const clicked = await api.contextMenu.show(
        [
          { id: "rename", label: "Rename thread" },
          { id: "mark-unread", label: "Mark unread" },
          { id: "pin", label: chatPinned ? "Unpin from Sidebar" : "Pin to Sidebar" },
      ...
      if (clicked === "pin") {
        if (chatPinned) {
          usePinnedItemsStore.getState().unpin(pinId);
        } else {
          usePinnedItemsStore.getState().pinChat({
          ...
          <Tooltip>
            <TooltipTrigger
              render={
                <button
                  type="button"
                  aria-label={`Open editor for ${project.displayName}`}
            ...
            />
            <TooltipPopup side="top">Open editor</TooltipPopup>
          </Tooltip>
          <Tooltip>
            <TooltipTrigger
              render={
...
function T3Wordmark() {
  return (
    <span
      aria-label="RF Code"
      className="shrink-0 select-none text-[13px] font-semibold leading-none tracking-tight text-foreground"
    >
      <span className="text-primary">RF</span>&nbsp;Code
    </span>
  );
}
