    ...
    (event: React.MouseEvent<HTMLButtonElement>) => {
      ...
      void (async () => {
        ...
        try {
          ...
          if (!connection) {
            throw new Error("Environment connection unavailable.");
          }
          ...
          await new Promise<void>((resolve, reject) => {
            ...
            const timeoutId = window.setTimeout(() => {
              ...
              settled = true;
              unsubscribe?.();
              reject(new Error("Timed out refreshing project state."));
            }, 5_000);

            unsubscribe = connection.client.orchestration.subscribeShell((item) => {
              if (settled || item.kind !== "snapshot") {
                return;
              }
              settled = true;
              window.clearTimeout(timeoutId);
              unsubscribe?.();
              useStore.getState().syncServerShellSnapshot(item.snapshot, project.environmentId);
              resolve();
            });
          });
