...
function buildVisibleMessageNodes(input: {
  ...
  const materializeThread = (threadId: ThreadId): VisibleMessageNode[] => {
    ...
    const prefixNodes = thread.messages.slice(0, replaceAtIndex).flatMap((message) => {
      if (message.role !== "user" && message.role !== "assistant") {
        return [
          {
            message,
...
function resolveLeafThreadId(input: {
  ...
  while (true) {
    ...
    for (const record of childRecords) {
      ...
      const messageIndex = messageIndexById.get(record.sourceMessageId) ?? Number.MAX_SAFE_INTEGER;
...
export default function ChatView({ threadId }: ChatViewProps) {
  ...
  const localDraftError = selectedServerThread
    ? null
  ...
  const visibleOriginThreadIdByMessageId = useMemo(
    () =>
      new Map(visibleMessageNodes.map((node) => [node.message.id, node.originThreadId] as const)),
    [visibleMessageNodes],
  );
  const visibleVariantSelectionByMessageId = useMemo(() => {
    const selectedByVariantKey = new Map<string, ThreadId>();
    for (const record of selectedBranchLineage.lineage) {
      selectedByVariantKey.set(
        `${record.variantGroupId}\u0000${record.anchorThreadId}`,
        record.threadId,
      );
    }
    ...
    const byMessageId = new Map<
      ...
      Pick<
        MessageVariantState,
        "variantGroupId" | "anchorThreadId" | "threadIds" | "currentThreadId" | "currentIndex"
      >
    >();
  ...
  const createNewTerminal = useCallback(() => {
    ...
    setTerminalFocusRequestId((value) => value + 1);
  }, [activeThreadId, hasReachedSplitLimit, storeNewTerminal]);
  ...
  const navigateToVisibleThread = useCallback(
    (nextThreadId: ThreadId) => {
      ...
      const targetRootThreadId =
        nextRootThreadId === routeRootThreadId ? routeRootThreadId : nextThreadId;
      const targetLeafThreadId =
        nextRootThreadId === routeRootThreadId ? nextThreadId : targetRootThreadId;
      void navigate({
        to: "/$threadId",
  ...
  const createBranchedThreadFromMessage = useCallback(
    async ({
      ...
      try {
        ...
        setVariantSelection(
          nextRecord.variantGroupId,
          nextRecord.anchorThreadId,
          nextRecord.threadId,
        );
  ...
  const resolveBranchSource = useCallback(
    (messageId: MessageId) => {
      ...
      const anchorMessage = anchorMessageId
  ...
  const onSubmitEditBranch = useCallback(() => {
    ...
    setEditBranchMessageId(null);
    setEditBranchText("");
  }, [createBranchedThreadFromMessage, editBranchMessageId, editBranchText, resolveBranchSource]);
  const onRetryAssistantMessage = useCallback(
    ...
    [createBranchedThreadFromMessage, resolveBranchSource, retryableAssistantMessageIds],
  );
      ...
      >
        <DialogPopup className="max-w-3xl">
          <DialogHeader>
            ...
            <DialogDescription>
              This creates a branched thread from the selected message. Existing image attachments
              are reused.
            </DialogDescription>
          </DialogHeader>
...
function buildBranchGraphRows(rootNode: BranchedThreadTreeNode): {
  rows: BranchGraphRow[];
  columnCount: number;
} {
...
const BranchGraph = memo(function BranchGraph({
  ...
  const { rows, columnCount } = useMemo(() => buildBranchGraphRows(rootNode), [rootNode]);
  ...
  return (
    <div className="space-y-2">
      {rows.map((row) => {
        ...
        const kindLabel = node.kind === "root" ? "Root" : node.kind === "edit" ? "Edit" : "Retry";
                  ...
                  <div className="mt-2 space-y-1 border-t border-border/60 pt-2">
                    {previews.map((preview) => (
                      <div key={preview.id} className="flex items-start gap-2 text-xs">
                        ...
                        <span className="truncate text-muted-foreground/80">{preview.preview}</span>
                      </div>
                    ))}
                  </div>
                )}
...
const MessagesTimeline = memo(function MessagesTimeline({
  ...
  const openFeedbackNote = useCallback(
    (message: TimelineMessage) => {
  ...
  useEffect(() => {
    ...
    onScrollToMessageHandled();
  }, [onScrollToMessageHandled, rowVirtualizer, rows, scrollToMessageId, virtualizedRowCount]);
  useEffect(() => {
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <MessageVariantControls
                      ...
                      onNavigate={(direction) =>
                        onNavigateMessageVariant(row.message.id, direction)
                      }
                    />
                  </div>
                ...
                <div className="mt-1.5 flex items-center justify-between gap-2">
                  <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      >
                        <ThumbsUpIcon
                          className={cn("size-3.5", feedbackFlags.up && "opacity-100")}
                        />
                      </Button>
                      <Button
                        type="button"
                      ...
                      >
                        <ThumbsDownIcon
                          className={cn("size-3.5", feedbackFlags.down && "opacity-100")}
                        />
                      </Button>
                    </div>
                    <Button
