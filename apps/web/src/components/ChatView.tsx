...
import {
  ...
  type BrowserAnnotationCaptureMessage,
...
export default function ChatView(props: ChatViewProps) {
  ...
  const sendBrowserAnnotation = useCallback(
    async (annotation: BrowserAnnotationCaptureMessage) => {
      const api = readEnvironmentApi(environmentId);
      if (
        !api ||
        !activeThread ||
        ...
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: "Could not send annotation",
            description: "The active chat is not ready to send a browser annotation.",
          }),
        );
        return;
      }

      const imageSizeBytes = estimateDataUrlByteSize(annotation.screenshotDataUrl);
      if (imageSizeBytes <= 0 || imageSizeBytes > PROVIDER_SEND_TURN_MAX_IMAGE_BYTES) {
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: "Could not send annotation",
            description: "The browser screenshot is too large to attach.",
          }),
        );
        return;
      }
      ...
      if (!sendCtx) {
        toastManager.add(
          stackedThreadToast({
            type: "error",
            title: "Could not send annotation",
            description: "The composer is not ready.",
          }),
        );
        return;
      }
      ...
      if (shouldCreateWorktree && !activeThreadBranch) {
        setThreadError(
          threadIdForSend,
          "Select a base branch before sending a browser annotation in New worktree mode.",
        );
        return;
      }

      const promptText = browserAnnotationPrompt(annotation);
      const messageIdForSend = newMessageId();
      const messageCreatedAt = new Date().toISOString();
      const outgoingMessageText = formatOutgoingPrompt({
        provider: ctxSelectedProvider,
      ...
      const attachmentName = `browser-annotation-${messageCreatedAt.replace(/[:.]/g, "-")}.png`;
      ...
      const optimisticAttachment = {
        type: "image" as const,
        id: `browser-annotation-${randomHex(8)}`,
        name: attachmentName,
        mimeType: "image/png",
        sizeBytes: imageSizeBytes,
        previewUrl: annotation.screenshotDataUrl,
      };
      ...
      let turnStartSucceeded = false;
      try {
        const titleSeed = annotation.text.trim() || "Browser annotation";
        const title = truncate(titleSeed);
        const threadCreateModelSelection = createModelSelection(
          ctxSelectedModelSelection.instanceId,
          ctxSelectedModel || activeProject.defaultModelSelection?.model || DEFAULT_MODEL,
        ...
        setThreadError(
          threadIdForSend,
          err instanceof Error ? err.message : "Failed to send browser annotation.",
        );
      } finally {
        sendInFlightRef.current = false;
        if (!turnStartSucceeded) {
  ...
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      ...
      void sendBrowserAnnotation(event.data);
    };
