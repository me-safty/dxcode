...
describe("canvasLayout", () => {
  ...
  it("keeps a dropped lane exactly at its drop point even with corridor insets", () => {
    // A back-edge forces a top corridor, which insets the auto-flow lanes.
    const looping = {
      name: "Loop",
      lanes: [
        { key: "queue", name: "Queue", entry: "manual", on: { success: "run" } },
    ...
    expect(initial.laneBandTop).toBeGreaterThan(0);
