  ...
  return (
    <>
      {actions.map((action) => {
        ...
        const routeBasePath = router.history.createHref(
          `/${environmentId}/p/${action.pluginId}`,
        );
        return (
          ...
          >
            {createElement(
              action.render as FunctionComponent<PluginProjectActionRenderProps>,
              { pluginId: action.pluginId, environmentId, projectId, projectName, routeBasePath },
            )}
          </PluginSurfaceErrorBoundary>
        );
      })}
