...
export function parseProjectSidebarRouteSearch(
  ...
  if (navProjectSort !== undefined) {
    parsed.navProjectSort = navProjectSort;
  }

  const navThreadSort = parseRouteEnum(search.navThreadSort, threadSortOrderValues);
  if (navThreadSort !== undefined) {
    parsed.navThreadSort = navThreadSort;
  }
  ...
  if (navTicketView !== undefined) {
    parsed.navTicketView = navTicketView;
  }

  const navThreads = parseRouteBoolean(search.navThreads);
  if (navThreads !== undefined) {
    parsed.navThreads = navThreads;
  }

  const navJira = parseRouteBoolean(search.navJira);
  if (navJira !== undefined) {
    parsed.navJira = navJira;
  }

  const navGitHub = parseRouteBoolean(search.navGitHub);
  if (navGitHub !== undefined) {
    parsed.navGitHub = navGitHub;
  }
