...
import { type GrokBuildSettings, type ModelSelection } from "@t3tools/contracts";
...
import { applyGrokAcpModelSelection, makeGrokAcpRuntime } from "../provider/acp/GrokAcpSupport.ts";
...
/**
 * Build a Grok Build text-generation closure bound to a specific provider
 * settings payload. See `makeCodexAdapter` for the per-instance rationale.
 */
  ...
  const runGrokBuildJson = <S extends Schema.Top>({
    ...
    Effect.gen(function* () {
          ...
          mapError: ({ cause, configId, step }) =>
            mapGrokBuildAcpError(
              ...
              step === "set-config-option"
                ? `Failed to set Grok Build ACP config option "${configId}" for text generation.`
                : "Failed to set Grok Build ACP base model for text generation.",
              cause,
            ),
        });
      ...
      Effect.mapError((cause) =>
        isTextGenerationError(cause)
          ? cause
          : mapGrokBuildAcpError(operation, "Grok Build ACP text generation failed.", cause),
      ),
      Effect.scoped,
    );
