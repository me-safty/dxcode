...
describe("task runtime worktree creation", () => {
  ...
  it.effect(
    "runs the project setup script runner after materializing orchestrator task worktrees",
    () =>
      Effect.gen(function* () {
        ...
        const response = yield* materializeTaskRuntime(request).pipe(Effect.provide(layer));
