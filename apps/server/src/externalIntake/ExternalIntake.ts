...
export class ExternalIntake extends Context.Service<
    ExternalIntake,
    ExternalIntakeShape
...
function resolvedProfileProject(
    ...
    return {
        ...
        title:
            profile.title ?? projectTitleFromWorkspace(profile.workspaceRoot),
    };
}
...
function projectResolutionErrorMessage(input: {
    ...
    const suffix =
        uniqueChoices.length > 0
            ? ` Available projects: ${uniqueChoices.slice(0, 8).join(", ")}.`
            : "";
...
function scriptListWithProfileScript(
    ...
    const setupScript =
        profile === null ? null : setupScriptToProjectScript(profile);
    ...
    return existing.map((script) =>
        script.id === setupScript.id ? setupScript : script,
    );
}

function scriptsEqual(
...
function buildInitialPrompt(input: ExternalIntakeMessage) {
    ...
    const supportAgentPrompt =
        ...
        envValue("SUPPORT_EMAIL_AGENT_PROMPT");

    if (input.source === "support_email") {
        return buildExternalIntakeInitialPrompt(input, {
            agentPrompt:
                supportAgentPrompt ?? DEFAULT_SUPPORT_EMAIL_AGENT_PROMPT,
...
function buildExternalIntakeInitialPrompt(
    ...
    const promptSections = [
        ...(triagePrompt
            ? [["<triage_prompt>", triagePrompt, "</triage_prompt>"].join("\n")]
            : []),
...
const makeExternalIntake = Effect.gen(function* () {
    ...
    const resolveProject = (input: ExternalIntakeMessage) =>
        Effect.gen(function* () {
            ...
            const hintText =
                `${input.projectHintText ?? ""}\n${input.text}`.toLowerCase();
            const profile = profiles.find((candidate) =>
                candidate.aliases.some((alias) =>
                    textMentionsAlias(hintText, alias),
                ),
            );
        ...
        Effect.gen(function* () {
            const modelSelection =
                ...
                (Option.isSome(input.existing)
                    ? input.existing.value.defaultModelSelection
                    : null) ??
            ...
            if (Option.isNone(input.existing)) {
                yield* orchestrationEngine.dispatch({
                    ...
                    commandId: CommandId.make(
                        `external-intake:project:create:${projectId}`,
                    ),
                    projectId,
                    title: input.title,
            ...
            const projectShell = Option.isSome(input.existing)
                ...
                : yield* projectionSnapshotQuery
                      ...
                      .pipe(
                          Effect.map(
                              (project) =>
                                  Option.getOrUndefined(project) ?? null,
                          ),
                      );
            const currentScripts = projectShell?.scripts ?? [];
            const nextScripts = scriptListWithProfileScript(
                currentScripts,
                input.profile,
            );
            if (
                ...
                yield* orchestrationEngine.dispatch({
                    ...
                    commandId: CommandId.make(
                        `external-intake:project:scripts:${projectId}`,
                    ),
                    projectId,
                    scripts: nextScripts,
                });
            ...
            yield* orchestrationEngine.dispatch({
                ...
                message: {
                    messageId: MessageId.make(
                        `external:${input.message.externalMessageId}:${messageNonce}`,
                    ),
                    role: "user",
    ...
    const createLinkedThread = (message: ExternalIntakeMessage, now: string) =>
        Effect.gen(function* () {
            ...
            yield* orchestrationEngine.dispatch({
                type: "thread.create",
                commandId: CommandId.make(
                    `external-intake:thread:create:${message.externalMessageId}`,
                ),
                threadId,
                projectId,
                title: message.title,
            ...
            yield* projectSetupScriptRunner
                ...
                .pipe(
                    Effect.catch((error) =>
                        Effect.logWarning(
                            "external intake failed to launch setup script",
                            {
            ...
            yield* orchestrationEngine.dispatch({
                type: "thread.turn.start",
                commandId: CommandId.make(
                    `external-intake:turn:start:${message.externalMessageId}`,
                ),
                threadId,
                message: {
                    messageId: MessageId.make(
            ...
            if (message.notificationSlackLink !== undefined) {
                yield* repository.upsertThreadLink({
                    source: "slack",
                    externalThreadId:
                        message.notificationSlackLink.externalThreadId,
                    t3ThreadId: threadId,
                    projectId,
                    primaryExternalMessageId:
                        message.notificationSlackLink.primaryExternalMessageId,
        ...
        Effect.gen(function* () {
            ...
            const mentionsBot =
                mentionsTeamAppUser({
                    body,
                    botUserId:
                        input.message.slack?.botUserId ??
                        envValue("SLACK_BOT_USER_ID"),
                    botUserName:
                        input.message.slack?.botUserName ??
                        envValue("SLACK_BOT_USERNAME"),
                }) ||
    ...
    const handleMessage: ExternalIntakeShape["handleMessage"] = (message) =>
        Effect.gen(function* () {
            ...
            const policyDecision = yield* handleSlackPolicy({
                message,
                existingMuted: Option.isSome(existing)
                    ? existing.value.muted
                    : false,
...
export const ExternalIntakeLive = Layer.effect(
    ExternalIntake,
    makeExternalIntake,
);
