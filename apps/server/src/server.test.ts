...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("streams and reconciles queued turn lifecycle events as thread detail events", () =>
    Effect.gen(function* () {
      const now = "2026-01-01T00:00:00.000Z";
      const threadId = ThreadId.make("thread-queued-detail");
      const messageId = MessageId.make("message-queued-detail");
      const queuedText = "queued while away";
      const queuedTurn: OrchestrationThread["queuedTurns"][number] = {
        threadId,
        messageId,
        role: "user",
        text: queuedText,
      ...
      const makeThread = (
        overrides: Pick<OrchestrationThread, "messages" | "queuedTurns" | "updatedAt">,
      ): OrchestrationThread => ({
        id: threadId,
        projectId: defaultProjectId,
        title: "Queued Detail Thread",
        modelSelection: defaultModelSelection,
        runtimeMode: "full-access",
        interactionMode: "default",
        branch: null,
      ...
      let detailSnapshotCallCount = 0;
      const queuedTurnDispatchedEvent = {
        sequence: 2,
        eventId: EventId.make("event-queued-turn-dispatched"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: "2026-01-01T00:00:01.000Z",
        commandId: CommandId.make("command-queued-turn-dispatched"),
      ...
      } satisfies Extract<OrchestrationEvent, { type: "thread.queued-turn-dispatched" }>;
      const messageSentEvent = {
        sequence: 3,
        eventId: EventId.make("event-queued-message-sent"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: "2026-01-01T00:00:02.000Z",
        commandId: CommandId.make("command-queued-message-sent"),
        causationEventId: null,
        correlationId: null,
        metadata: {},
        ...
        layers: {
          projectionSnapshotQuery: {
            getThreadDetailSnapshotById: () =>
              Effect.sync(() => {
                detailSnapshotCallCount += 1;
                return Option.some(
                  detailSnapshotCallCount === 1 ? queuedSnapshot : promotedSnapshot,
                );
              }),
          },
          orchestrationEngine: {
            readEvents: (fromSequenceExclusive) =>
              Stream.fromIterable([queuedTurnDispatchedEvent, messageSentEvent]).pipe(
                Stream.filter((event) => event.sequence > fromSequenceExclusive),
              ),
          },
        },
      });
      ...
      assert.equal(subscriptionItems.length, 3);
      const firstItem = subscriptionItems[0];
      const secondItem = subscriptionItems[1];
      const thirdItem = subscriptionItems[2];
      if (firstItem?.kind !== "snapshot") {
        assert.fail("Expected subscribeThread to start with a snapshot");
      }
      assert.equal(firstItem.snapshot.thread.queuedTurns[0]?.text, queuedText);
      if (secondItem?.kind !== "event" || thirdItem?.kind !== "event") {
        assert.fail("Expected subscribeThread to replay queued dispatch and message events");
      }
      assert.deepEqual(
        [secondItem.event.type, thirdItem.event.type],
        ["thread.queued-turn-dispatched", "thread.message-sent"],
      );

      const missingVerificationResult = yield* Effect.scoped(
        withWsRpcClient(wsUrl, (client) =>
          client[ORCHESTRATION_WS_METHODS.reconcileThreadDetail]({
            threadId,
            clientSequence: null,
      ...
      if (missingVerificationResult.kind !== "snapshot") {
        assert.fail("Expected missing client verification to return a snapshot");
      }
      assert.equal(missingVerificationResult.snapshot.thread.messages[0]?.text, queuedText);
      assert.deepEqual(missingVerificationResult.snapshot.thread.queuedTurns, []);
      ...
      const reconcileResult = yield* Effect.scoped(
        withWsRpcClient(wsUrl, (client) =>
          client[ORCHESTRATION_WS_METHODS.reconcileThreadDetail]({
            ...
            clientSequence: 1,
            verifiedSequence: 1,
            verifiedFingerprint: computeOrchestrationThreadDetailFingerprint(queuedSnapshot),
          }),
        ),
      );
      if (reconcileResult.kind !== "events") {
        assert.fail("Expected reconcileThreadDetail to replay detail events");
      }
      assert.deepEqual(
