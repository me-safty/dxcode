...
it.layer(NodeServices.layer)("server router seam", (it) => {
  ...
  it.effect("routes websocket rpc git.stashAndCheckout errors after refreshing git status", () =>
    Effect.gen(function* () {
      const gitError = new GitCommandError({
        operation: "stashAndCheckout",
      ...
      yield* buildAppUnderTest({
        layers: {
          gitCore: {
            stashAndCheckout: () => Effect.fail(gitError),
          },
          gitStatusBroadcaster: {
            refreshStatus: (cwd) =>
              Effect.sync(() => {
      ...
      const result = yield* Effect.scoped(
        withWsRpcClient(wsUrl, (client) =>
          client[WS_METHODS.gitStashAndCheckout]({
            cwd: "/tmp/repo",
            branch: "feature/demo",
          }).pipe(Effect.result),
        ),
