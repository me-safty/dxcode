...
const buildAppUnderTest = (options?: {
  ...
  layers?: {
    ...
    providerUsage?: Partial<ProviderUsage.ProviderUsageService["Service"]>;
  ...
  Effect.gen(function* () {
    ...
    const servedRoutesLayer = HttpRouter.serve(makeRoutesLayer, {
      ...
      Layer.provide(
        // Merged into one `Layer.provide` rather than a second chained call:
        // this `.pipe(...)` sits at its typed 20-argument overload limit, so an
        // extra provide collapses the whole layer's type to `unknown`.
        Layer.mergeAll(
          ...
          Layer.mock(ProviderUsage.ProviderUsageService)({
            getUsage: () => Effect.succeed({ usage: [] }),
            ...options?.layers?.providerUsage,
          }),
        ),
      ),
      Layer.provide(
