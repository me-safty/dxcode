...
const baseDirFlag = Flag.string("base-dir").pipe(
  Flag.withDescription("Base directory path (equivalent to WORKBENCH_HOME)."),
  Flag.optional,
);
...
const logWebSocketEventsFlag = Flag.boolean("log-websocket-events").pipe(
  Flag.withDescription(
    "Emit server-side logs for outbound WebSocket push traffic (equivalent to WORKBENCH_LOG_WS_EVENTS).",
  ),
...
const booleanConfigWithAliases = (...names: ReadonlyArray<string>) =>
  optionalConfigWithAliases(names, Config.boolean);
const intConfigWithAliases = (...names: ReadonlyArray<string>) =>
  optionalConfigWithAliases(names, Config.int);
const portConfigWithAliases = (...names: ReadonlyArray<string>) =>
  optionalConfigWithAliases(names, Config.port);

const EnvServerConfig = Config.all({
  logLevel: Config.orElse(Config.logLevel("WORKBENCH_LOG_LEVEL"), () =>
    Config.logLevel("T3CODE_LOG_LEVEL"),
  ).pipe(Config.withDefault("Info")),
  traceMinLevel: Config.orElse(Config.logLevel("WORKBENCH_TRACE_MIN_LEVEL"), () =>
    Config.logLevel("T3CODE_TRACE_MIN_LEVEL"),
  ).pipe(Config.withDefault("Info")),
  traceTimingEnabled: Config.orElse(Config.boolean("WORKBENCH_TRACE_TIMING_ENABLED"), () =>
    Config.boolean("T3CODE_TRACE_TIMING_ENABLED"),
  ).pipe(Config.withDefault(true)),
  traceFile: stringConfigWithAliases("WORKBENCH_TRACE_FILE", "T3CODE_TRACE_FILE"),
  traceMaxBytes: Config.orElse(Config.int("WORKBENCH_TRACE_MAX_BYTES"), () =>
    Config.int("T3CODE_TRACE_MAX_BYTES"),
  ).pipe(Config.withDefault(10 * 1024 * 1024)),
  traceMaxFiles: Config.orElse(Config.int("WORKBENCH_TRACE_MAX_FILES"), () =>
    Config.int("T3CODE_TRACE_MAX_FILES"),
  ).pipe(Config.withDefault(10)),
  traceBatchWindowMs: Config.orElse(Config.int("WORKBENCH_TRACE_BATCH_WINDOW_MS"), () =>
    Config.int("T3CODE_TRACE_BATCH_WINDOW_MS"),
  ).pipe(Config.withDefault(200)),
  otlpTracesUrl: stringConfigWithAliases("WORKBENCH_OTLP_TRACES_URL", "T3CODE_OTLP_TRACES_URL"),
  otlpMetricsUrl: stringConfigWithAliases("WORKBENCH_OTLP_METRICS_URL", "T3CODE_OTLP_METRICS_URL"),
  otlpExportIntervalMs: Config.orElse(Config.int("WORKBENCH_OTLP_EXPORT_INTERVAL_MS"), () =>
    Config.int("T3CODE_OTLP_EXPORT_INTERVAL_MS"),
  ).pipe(Config.withDefault(10_000)),
  otlpServiceName: Config.orElse(Config.string("WORKBENCH_OTLP_SERVICE_NAME"), () =>
    Config.string("T3CODE_OTLP_SERVICE_NAME"),
  ).pipe(Config.withDefault("workbench-server")),
  mode: Config.orElse(Config.schema(RuntimeMode, "WORKBENCH_MODE"), () =>
    Config.schema(RuntimeMode, "T3CODE_MODE"),
  ).pipe(Config.option, Config.map(Option.getOrUndefined)),
  port: portConfigWithAliases("WORKBENCH_PORT", "T3CODE_PORT"),
  host: stringConfigWithAliases("WORKBENCH_HOST", "T3CODE_HOST"),
  t3Home: stringConfigWithAliases("WORKBENCH_HOME", "T3CODE_HOME"),
  devUrl: Config.url("VITE_DEV_SERVER_URL").pipe(Config.option, Config.map(Option.getOrUndefined)),
  noBrowser: booleanConfigWithAliases("WORKBENCH_NO_BROWSER", "T3CODE_NO_BROWSER"),
  bootstrapFd: intConfigWithAliases("WORKBENCH_BOOTSTRAP_FD", "T3CODE_BOOTSTRAP_FD"),
  autoBootstrapProjectFromCwd: booleanConfigWithAliases(
    "WORKBENCH_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
    "T3CODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD",
  ),
  logWebSocketEvents: booleanConfigWithAliases("WORKBENCH_LOG_WS_EVENTS", "T3CODE_LOG_WS_EVENTS"),
});
...
const startCommand = Command.make("start", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription("Run the Workbench server."),
  Command.withHandler((flags) => runServerCommand(flags)),
);

const serveCommand = Command.make("serve", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription(
    "Run the Workbench server without opening a browser and print headless pairing details.",
  ),
  Command.withHandler((flags) =>
...
export const cli = Command.make("workbench", { ...sharedServerCommandFlags }).pipe(
  Command.withDescription("Run the Workbench server."),
  Command.withHandler((flags) => runServerCommand(flags)),
