...
import { classifyDeltas, hashContent, type MappingRow } from "./sourceReconcileDiff.ts";
...
describe("classifyDeltas", () => {
  ...
  it('Fix 1: an upstream description cleared (non-empty → empty) produces a changed delta that CLEARS the description to ""', () => {
    ...
    // Next cycle: the mapping now stores the cleared hash → no further delta.
    const noop = classifyDeltas({
      ...defaultInput,
      items: [item],
      mappings: [
        makeMapping({ contentHash: hashContent({ title: "Fix the bug", description: "" }) }),
      ],
      scanCompleted: true,
    });
