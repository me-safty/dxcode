  ...
  Effect.gen(function* () {
    ...
    const buildTicketUrl = (
      ...
      webBaseUrl === undefined
        ? undefined
        : `${webBaseUrl}/tickets/${encodeURIComponent(
            envId,
          )}/${encodeURIComponent(boardId)}/${encodeURIComponent(ticketId)}`;
    ...
    const processRow = (row: DeliveryRow, envId: EnvironmentId): Effect.Effect<void> =>
      Effect.gen(function* () {
        ...
        const response = yield* client.execute(request).pipe(
          Effect.mapError((cause) => retryable(`HTTP network error: ${String(cause)}`)),
        ...
        // ignored; the status has already been read.
