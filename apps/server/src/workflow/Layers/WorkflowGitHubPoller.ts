...
const makeWorkflowGitHubPoller = (options?: WorkflowGitHubPollerLiveOptions) =>
  Effect.gen(function* () {
    ...
    const rotateTickets = (rows: ReadonlyArray<WatchedTicketRow>) => {
      if (nextSweepCursorTicketId === null) {
        return rows;
      }
      const startIndex = rows.findIndex(
        (row) => (row.ticketId as string) === nextSweepCursorTicketId,
      );
      if (startIndex <= 0) {
        return rows;
      }
    ...
    const observeTicket = (ticket: WatchedTicketRow) =>
      Effect.gen(function* () {
        ...
        if (detail.reviewDecision !== ticket.lastReviewDecision) {
          if (detail.reviewDecision === "changes_requested") {
            // Post each NEW feedback item (newer than the stored cursor) as its
            // own observation with a body; emit ONE routing event.
            ...
            const fresh = feedback.filter((item) => cursor === null || item.submittedAt > cursor);
            ...
            // The routing event itself (no body) — deduped by head sha so a new
            // review round on a new push re-fires.
            yield* push({
              dedupKey: `review:${tid}:${detail.headSha ?? "nohead"}:changes_requested`,
              eventName: "pr.changes_requested",
              payload: {},
              messageBody: null,
            });
          ...
          } else if (detail.reviewDecision === "approved") {
            yield* push({
      ...
      Effect.gen(function* () {
        ...
        const ingestOutcome = yield* engine
          ...
          .pipe(
            ...
            Effect.catch((error) =>
              error.message === "ticket not found on this board"
