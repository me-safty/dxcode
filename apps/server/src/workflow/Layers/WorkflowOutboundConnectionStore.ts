...
const makeWithDeps = (validatorDeps: UrlValidatorDeps | undefined) =>
  Effect.gen(function* () {
    ...
    const create: WorkflowOutboundConnectionStoreShape["create"] = Effect.fn(
      ...
      // Store secret AFTER row insert so that a failed insert leaves no
      // orphaned secret. A row-exists-but-secret-missing state is still
      // graceful: getTarget will fail with OutboundConfigError at delivery
      // time, and remove can still clean up the row.
      yield* sql`
        ...
        `.pipe(Effect.mapError(toOutboundConfigError("Failed to insert outbound connection")));
