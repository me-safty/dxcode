...
const RealPathProviderDoubleLive = Layer.unwrap(
  Effect.gen(function* () {
    ...
    const appendInstruction = (request: DispatchRequest) =>
      Effect.gen(function* () {
        const outputPath = path.join(request.worktreePath, "workflow-output.txt");
...
> =>
  Effect.gen(function* () {
    ...
    yield* writeTextFile(path.join(cwd, "README.md"), "# workflow repo\n");
    yield* git(cwd, ["add", "."]);
...
const registerBoardProjection = (input: {
  ...
  Effect.gen(function* () {
    ...
    yield* read.registerBoard({
      ...
      name: input.name,
      workflowFilePath: path.join(input.repoRoot, ".t3", "boards", "delivery.json"),
...
describe.sequential("Workflow runtime real path", () => {
  it.effect("runs a two-step agent pipeline in one project worktree with accumulated diff", () =>
    Effect.gen(function* () {
      ...
      yield* makeDirectory(path.join(targetRepo, "prompts"));
      yield* writeTextFile(path.join(targetRepo, "prompts", "step-one.md"), "first file prompt");
      ...
      assert.isTrue((calls[0]?.worktreePath ?? "").includes(path.basename(targetRepo)));
