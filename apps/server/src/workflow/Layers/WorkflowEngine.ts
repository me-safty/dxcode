...
const make = Effect.gen(function* () {
  ...
  const boardIdForStepRun = (stepRunId: StepRunId) =>
    wrapSql(sql<StepBoardRow>`
      SELECT ticket.board_id AS "boardId"
      ...
      UNION ALL
      SELECT json_extract(created.payload_json, '$.boardId') AS "boardId"
      FROM workflow_events AS awaiting
      INNER JOIN workflow_events AS created
        ON created.ticket_id = awaiting.ticket_id
        AND created.event_type = 'TicketCreated'
      WHERE awaiting.event_type = 'StepAwaitingUser'
        AND json_extract(awaiting.payload_json, '$.stepRunId') = ${stepRunId}
      LIMIT 1
  ...
  const resolveApproval: WorkflowEngineShape["resolveApproval"] = (stepRunId, approved) =>
    Effect.gen(function* () {
      ...
      const boardId = yield* boardIdForStepRun(stepRunId);
      if (boardId === null) {
        yield* resolve;
        return;
      }
