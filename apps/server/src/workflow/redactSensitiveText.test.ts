...
describe("redactSensitiveText", () => {
  describe("GitHub tokens", () => {
    it("redacts ghp_ tokens", () => {
      assert.equal(
        redactSensitiveText("token: ghp_abcdefghijklmnopqrstu1234567890"),
        "token: [redacted]",
      );
    });
    ...
    it("redacts gho_ tokens via the gh prefix pattern (not high-entropy)", () => {
      ...
      assert.equal(redactSensitiveText("auth gho_abcdefghij1234567890ab"), "auth [redacted]");
    });
    ...
    it("redacts github_pat_ tokens", () => {
      assert.equal(
        redactSensitiveText("github_pat_ABCDEFGHIJKLMNOPQRSTUVWXYZabc1234567890"),
        "[redacted]",
      );
    });
  });
  ...
  describe("AWS keys", () => {
    it("redacts AKIA keys", () => {
      assert.equal(redactSensitiveText("key: AKIAIOSFODNN7EXAMPLE"), "key: [redacted]");
    });
