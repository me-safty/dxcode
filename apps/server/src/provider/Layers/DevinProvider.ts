...
function isDevOnlyDevinModelName(name: string): boolean {
  return name.trim().endsWith("[dev]");
}
...
export function buildDevinDiscoveredModelsFromConfigOptions(
  ...
  const flattened = modelOption.options.flatMap((entry) =>
  ...
  return flattened
    .map((entry): ServerProviderModel | undefined => {
      ...
      if (!slug || seen.has(slug) || isDevOnlyDevinModelName(name)) {
        return undefined;
      }
      seen.add(slug);
      return {
...
const discoverDevinModelsViaAcp = (
  ...
  Effect.gen(function* () {
    ...
    return buildDevinDiscoveredModelsFromConfigOptions(started.sessionSetupResult.configOptions);
  }).pipe(Effect.scoped);
...
export function parseDevinAuthStatusOutput(output: string): ServerProviderAuth {
  if (/^\s*Logged in\b/m.test(output)) {
    const email = /^\s*Email:\s*(\S+)\s*$/m.exec(output)?.[1];
    return {
      status: "authenticated",
      ...(email ? { email } : {}),
    };
  }
  if (/^\s*Not logged in\b/m.test(output) || /Failed to load credentials/.test(output)) {
    return { status: "unauthenticated" };
  }
  return { status: "unknown" };
}

const probeDevinAuthStatus = (
  devinSettings: DevinSettings,
...
export const checkDevinProviderStatus = Effect.fn("checkDevinProviderStatus")(function* (
  ...
  const auth = yield* probeDevinAuthStatus(devinSettings, environment);
  if (auth.status === "unauthenticated") {
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "warning",
        auth,
        message: `Devin CLI is installed but not logged in. ${DEVIN_LOGIN_HINT}`,
      },
    });
  }
