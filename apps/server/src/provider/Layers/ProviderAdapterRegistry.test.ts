...
import { FactoryDroidAdapter, FactoryDroidAdapterShape } from "../Services/FactoryDroidAdapter.ts";
  ...
  Layer.mergeAll(
    Layer.provide(
      ...
      Layer.mergeAll(
        ...
        Layer.succeed(FactoryDroidAdapter, fakeFactoryDroidAdapter),
      ),
    ),
    NodeServices.layer,
  ),
...
layer("ProviderAdapterRegistryLive", (it) => {
  it.effect("resolves a registered provider adapter", () =>
    Effect.gen(function* () {
      ...
      const droid = yield* registry.getByProvider("factoryDroid");
      assert.equal(codex, fakeCodexAdapter);
      assert.equal(claude, fakeClaudeAdapter);
      assert.equal(droid, fakeFactoryDroidAdapter);

      const providers = yield* registry.listProviders();
      assert.deepEqual(providers, ["codex", "claudeAgent", "factoryDroid"]);
    }),
  );
