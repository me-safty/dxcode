...
it.live("recovers claudeCode sessions after adapter stopAll using persisted resume state", () =>
  withHarness(
    (harness) =>
      Effect.gen(function* () {
        yield* seedProjectAndThread(harness);
  ...
  withHarness(
    (harness) =>
      Effect.gen(function* () {
        ...
        yield* harness.waitForThread(THREAD_ID, (entry) => entry.session?.threadId === "thread-1");
