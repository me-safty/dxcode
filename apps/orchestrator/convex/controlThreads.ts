...
import { linearThreadKeyFor } from "../src/linear/ingress.ts";
...
export const upsertFromLinearIngress = internalMutation({
  ...
  handler: async (ctx, args) => {
    const threadKey = linearThreadKeyFor(args);
    const now = Date.now();
    ...
    if (existingEvent !== null) {
      ...
      return {
        controlThreadId: controlThread._id,
        threadKey: controlThread.linearThreadKey,
        created: false,
      };
    }
    ...
    const existingThread = await ctx.db
      ...
      .withIndex("by_linear_thread_key", (query: any) => query.eq("linearThreadKey", threadKey))
      .unique();
    ...
    const existingMessage = await ctx.db
      ...
      .withIndex("by_external_message_key", (query: any) =>
        query.eq("externalMessageKey", args.eventId),
      )
      .unique();
    ...
    if (existingMessage === null) {
      await ctx.db.insert("controlThreadMessages", {
        controlThreadId: nextThread,
        externalMessageKey: args.eventId,
        createdAt: now,
        updatedAt: now,
