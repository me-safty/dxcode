...
export async function preflightRemoteConnection(connection: RemoteConnectionConfig): Promise<void> {
  ...
  try {
    ...
    if (error instanceof Error && error.name === "AbortError") {
      throw new Error(
        "Timed out reaching T3 desktop. Make sure the app is open and remote is enabled.",
        { cause: error },
      );
    }
    throw new Error(
      "Could not reach T3 desktop. Make sure the desktop app is open and the remote URL is correct.",
      { cause: error },
    );
  } finally {
  ...
  if (response.status === 401) {
    throw new Error(
      "Remote access rejected the auth token. Copy the token from desktop Settings -> Enable remote.",
    );
  }

  if (response.status === 503) {
    throw new Error(
      "T3 desktop is open, but its backend is not ready yet. Wait a moment and try again.",
    );
  }
