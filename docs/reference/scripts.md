...
- `vp run sync:upstream -- --dry-run` — Detects upstream commits; without dry-run creates an
...
- `vp run dist:desktop:dx:dmg` — Builds isolated `DX Code` macOS artifacts into `./release-dx`.
