...
import { WorkflowDefinition, WorkflowRpcError } from "../../../contracts/workflow.ts";
import { AsanaSelector, GithubSelector, JiraSelector } from "../../../contracts/workSource.ts";
...
export const WorkflowProviderInstancePortLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const providerInstanceExists = (instanceId: string) =>
      agents.listInstances().pipe(
        Effect.map((instances) =>
          instances.available.some((instance) => String(instance.instanceId) === instanceId),
        ),
        Effect.orElseSucceed(() => false),
      );
    return {
      providerInstanceExists,
      providerInstanceSupportsResume: providerInstanceExists,
