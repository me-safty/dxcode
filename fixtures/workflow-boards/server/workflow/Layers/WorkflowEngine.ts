...
const make = Effect.gen(function* () {
  ...
  const usageReader = yield* StepUsageReader;
      ...
      yield* Effect.forEach(
        ...
        (threadId) =>
          providerCleanupAttempt(
            agentPort.cleanupSession(threadId as ThreadId),
  ...
  const tearDownTicketAgentSessions = (ticketId: TicketId, stopProviderSessions: boolean) =>
    Effect.gen(function* () {
      if (stopProviderSessions) {
        const rows = yield* agentSessions
          .listByTicket(ticketId)
          .pipe(Effect.orElseSucceed(() => []));
        yield* stopAgentSessionThreads(rows.map((row) => row.threadId));
      }
      yield* agentSessions.deleteByTicket(ticketId).pipe(Effect.catch(() => Effect.void));
    });
  ...
  const readStoredEventsForStep = (stepRunId: StepRunId) =>
    Effect.gen(function* () {
      ...
      return yield* Stream.runCollect(store.readByTicket(ticketId)).pipe(
  ...
  const ensureLiveProviderUserInputWait = (
    ...
    Effect.gen(function* () {
      ...
      const live = yield* agentPort.isPendingRequestLive({
      ...
      if (!live) {
        return yield* new WorkflowEventStoreError({
          message: "provider user-input request is no longer pending",
        });
      }
    });
  ...
  const readStepUsage = (
    ...
    Effect.gen(function* () {
      ...
      return yield* usageReader.read(threadId);
    });
  ...
  const awaitProviderTerminalForStep = (
    ...
    Effect.gen(function* () {
      const result = yield* agentPort.awaitStepTerminal(stepRunId, threadId);
  ...
  const completedResultForStep = (
    ...
    Effect.gen(function* () {
      ...
      const dispatch = yield* agentPort.getDispatchForStep(stepRunId);
      const threadId = captureTurn?.threadId ?? dispatch?.threadId;
      const messageId = dispatch?.messageId;
      ...
      return yield* agentPort.readCapturedOutput({ stepRunId, threadId, messageId }).pipe(
        Effect.map((captured) => {
  ...
  const cancellableProviderTurnsForTicket: WorkflowEngineShape["cancellableProviderTurnsForTicket"] =
    (ticketId) =>
      cancellableProviderDispatchesForTicket(ticketId).pipe(
        Effect.map((rows) =>
          rows.map((row) => ({ threadId: row.threadId, messageId: row.messageId })),
        ),
      );
  ...
  const cancellableProviderDispatchesForBoard = (boardId: BoardId) =>
    wrapSql(sql<ActiveProviderTurnRow>`
      ...
      ORDER BY outbox.thread_id ASC, outbox.message_id ASC
    `);
  ...
  const cancellableProviderDispatchesForTicket = (ticketId: TicketId) =>
    wrapSql(sql<ActiveProviderTurnRow>`
      ...
      WHERE ticket_id = ${ticketId}
        AND status IN ('reserved', 'start_requested', 'projected')
  ...
  const cancelProviderTurns = (turns: ReadonlyArray<ActiveProviderTurnRow>) =>
    Effect.gen(function* () {
      yield* Effect.forEach(
        ...
        (turn) =>
          Effect.gen(function* () {
            const cleanupError = yield* providerCleanupAttempt(
              agentPort.cleanupSession(turn.threadId),
