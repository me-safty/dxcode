import { WorkflowEvent } from "../../../contracts/workflow.ts";
...
const make = Effect.gen(function* () {
  ...
  // (ticket_id, stream_version) UNIQUE index (p_workflow_boards_idx_workflow_events_stream_version,
  ...
  const deleteForBoard: WorkflowEventStoreShape["deleteForBoard"] = (boardId) =>
    sql`
      DELETE FROM p_workflow_boards_events
      WHERE ticket_id IN (
        SELECT ticket_id
