...
const layer = it.layer(TestSql);

layer("workflow migration", (it) => {
  it.effect("creates p_workflow_boards_events and projection tables", () =>
    Effect.gen(function* () {
      const sql = yield* SqlClient.SqlClient;
      const tables = yield* sql<{ readonly name: string }>`
        SELECT name FROM sqlite_master WHERE type = 'table'
        AND name IN (
          'p_workflow_boards_events',
          'p_workflow_boards_projection_board',
          'p_workflow_boards_projection_ticket',
          'p_workflow_boards_projection_pipeline_run',
          'p_workflow_boards_projection_step_run'
        )
      `;
...
const storeLayer = it.layer(WorkflowEventStoreLive.pipe(Layer.provideMerge(TestSql)));
