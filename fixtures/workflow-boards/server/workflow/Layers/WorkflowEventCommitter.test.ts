...
import { NotificationSink, type NotificationAttention } from "../NotificationSink.ts";
...
const capturedNotifications: NotificationAttention[] = [];

const resetCapturedNotifications = () => {
  capturedNotifications.splice(0);
};

const notificationsForTicket = (ticketId: string) =>
  Effect.sync(() =>
    capturedNotifications.filter((notification) => notification.ticketId === ticketId),
  );

const notificationCount = (ticketId: string) =>
  Effect.map(notificationsForTicket(ticketId), (notifications) => notifications.length);

const CapturingNotificationSink = Layer.succeed(NotificationSink, {
...
const layer = it.layer(
  WorkflowEventCommitterLive.pipe(
    ...
    Layer.provideMerge(CapturingNotificationSink),
...
layer("WorkflowEventCommitter", (it) => {
  it.effect("appends and projects in one call", () =>
    Effect.gen(function* () {
      ...
      const rows = yield* sql<{ readonly title: string }>`
        SELECT title FROM p_workflow_boards_projection_ticket WHERE ticket_id = 't-1'
      `;
  ...
  it.effect("notifies once when an event flips a ticket into waiting_on_user", () =>
    Effect.gen(function* () {
      resetCapturedNotifications();
      ...
      const rows = yield* notificationsForTicket(ticketId);
      ...
      const eventRows = yield* (yield* SqlClient.SqlClient)<{ readonly sequence: number }>`
          SELECT sequence FROM p_workflow_boards_events WHERE ticket_id = ${ticketId}
        `;
      assert.equal(row.sequence, eventRows[0]?.sequence);
    }),
  );

  it.effect("notifies when a ticket is blocked", () =>
    Effect.gen(function* () {
      resetCapturedNotifications();
  ...
  it.effect("does not notify when an event does not cross into a needs-you state", () =>
    Effect.gen(function* () {
      resetCapturedNotifications();
      ...
      assert.equal(yield* notificationCount(ticketId), 0);
    }),
  );

  it.effect("does not notify again when the ticket stays in the same needs-you status", () =>
    Effect.gen(function* () {
      resetCapturedNotifications();
      ...
      assert.equal(yield* notificationCount(ticketId), 1);
  ...
  it.effect("notifies once per distinct needs-you transition", () =>
    Effect.gen(function* () {
      resetCapturedNotifications();
      ...
      // 1) waiting_on_user → first attention notification
      yield* committer.commit({
        type: "StepAwaitingUser",
      ...
      // 2) blocked → second attention notification (no dedup: the sink emits one per transition)
      yield* committer.commit({
        type: "TicketBlocked",
      ...
      assert.equal(rows[0]?.attentionReason, "approve deploy?");
      assert.equal(rows[1]?.status, "blocked");
      assert.equal(rows[1]?.attentionKind, "blocked");
      assert.equal(rows[1]?.attentionReason, "merge conflict");
      assert.isAbove(rows[1]!.sequence, rows[0]!.sequence);
    }),
  );
});

it.effect(
  "rolls back the event and does not notify when projection fails inside the commit transaction",
  () =>
    Effect.gen(function* () {
      resetCapturedNotifications();
      ...
      // p_workflow_boards_projection_board) then seed a running p_workflow_boards_projection_ticket row.
      yield* registerBoard(boardId);
      ...
      // notification from being emitted (single-commit path is transactional).
      assert.equal(yield* workflowEventCount(ticketId), 0);
      assert.equal(yield* notificationCount(ticketId), 0);
    }).pipe(
      Effect.provide(
