...
const layer = it.layer(
  WorkflowEventCommitterLive.pipe(
    ...
    Layer.provideMerge(NotificationSinkNoop),
...
const deliveryRows = (ticketId: string) =>
  Effect.gen(function* () {
    ...
    return yield* sql<DeliveryRow>`
      ...
      FROM p_workflow_boards_outbound_delivery
...
const eventSequence = (ticketId: string) =>
  Effect.gen(function* () {
    ...
    const rows = yield* sql<{ readonly sequence: number }>`
      SELECT sequence FROM p_workflow_boards_events WHERE ticket_id = ${ticketId} ORDER BY sequence ASC
    `;
    return rows;
  });
...
layer("WorkflowEventCommitter outbound", (it) => {
  ...
  it.effect("re-running the same supersede+insert for one sequence stays exactly-once", () =>
    Effect.gen(function* () {
      ...
      const sql = yield* SqlClient.SqlClient;
      yield* sql`
        INSERT OR IGNORE INTO p_workflow_boards_outbound_delivery (
...
it.effect("a predicate-eval error skips the rule without failing the commit", () =>
  Effect.gen(function* () {
    ...
    const tickets = yield* sql<{ readonly status: string }>`
        SELECT status FROM p_workflow_boards_projection_ticket WHERE ticket_id = ${ticketId}
      `;
