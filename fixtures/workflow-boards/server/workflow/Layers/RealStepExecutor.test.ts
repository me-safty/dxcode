...
const WorkflowFilesystemCapabilityFake = Layer.succeed(WorkflowFilesystemCapability, {
  listRoots: () => Effect.succeed(["/tmp/wt-ticket-1"]),
  ...
  stat: () => Effect.succeed({ type: "directory", size: 0, mtime: 0 }),
...
const ScriptStepExecutorTestLive = ScriptStepExecutorLive.pipe(
  Layer.provideMerge(WorkflowFilesystemCapabilityFake),
  Layer.provideMerge(Path.layer),
);

const mk = (
  terminal: WorkflowDispatchTerminalResult,
  options: {
  ...
  it.layer(
    RealStepExecutorLive.pipe(
      ...
      Layer.provideMerge(ScriptStepExecutorTestLive),
      Layer.provideMerge(
      ...
      Layer.provideMerge(
        Layer.succeed(WorkflowAgentPort, {
          ensureStarted: (input) =>
            Effect.sync(() => {
              dispatchStartInputs.push(input);
              return { messageId: "message-stub" as never };
            }),
          ...
          respond: () => Effect.void,
          isPendingRequestLive: () => Effect.succeed(false),
      ...
      Layer.provideMerge(
        Layer.succeed(WorkflowAgentPort, {
          ensureStarted: (input) =>
            Effect.sync(() => {
              dispatchStartInputs.push(input);
              return { messageId: "message-capture" as never };
            }),
...
).pipe(Layer.provide(TestNodeFileSystemLive));
...
const seedHandoffStepRun = (input: {
  ...
  Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient;
    const outputJson = yield* encodeUnknownJson(input.output);
    yield* sql`
...
mk({ ok: true })("RealStepExecutor description spill (120k provider)", (it) => {
  ...
  it.effect(
    "spills an over-budget description to DESCRIPTION.md and replaces all occurrences",
    () =>
      Effect.gen(function* () {
        dispatchStartInputs.length = 0;
        const executor = yield* StepExecutor;
        const fileSystem = yield* FileSystem.FileSystem;
        const longDescription = "D".repeat(130_000);
  ...
  it.effect("spills both the description and an over-budget handoff", () =>
    Effect.gen(function* () {
      dispatchStartInputs.length = 0;
      const executor = yield* StepExecutor;
      const fileSystem = yield* FileSystem.FileSystem;
      const longDescription = "D".repeat(130_000);
      const longReview = "R".repeat(130_000);
      ...
      assert.isAtMost(dispatched.instruction.length, 120_000);
      yield* fileSystem
  ...
  it.effect(
    "spills an inlinable description when handoff pointer overhead pushes it over budget",
    () =>
      Effect.gen(function* () {
        dispatchStartInputs.length = 0;
        const executor = yield* StepExecutor;
        const fileSystem = yield* FileSystem.FileSystem;
        const inlinableDescription = "D".repeat(119_900);
        const longReview = "R".repeat(70_000);
        const ctx = {
...
captureLayer({ verdict: "pass", score: 0.98 })("RealStepExecutor output capture", (it) => {
  it.effect("appends the capture instruction, persists it, and returns the last JSON block", () =>
    Effect.gen(function* () {
      ...
      const dispatched = dispatchStartInputs[0] as { readonly instruction: string };
      assert.include(dispatched.instruction, "Implement the ticket");
      assert.include(
        dispatched.instruction,
  ...
  it.effect("passes the exact started thread and message to the output reader", () =>
    Effect.gen(function* () {
      capturedReadInputs.length = 0;
      dispatchStartInputs.length = 0;
      const executor = yield* StepExecutor;
      ...
      const dispatched = dispatchStartInputs[0] as
        | { readonly threadId: string; readonly stepRunId: string }
        | undefined;
...
const preCheckpointFailureLayer = it.layer(
  RealStepExecutorLive.pipe(
    ...
    Layer.provideMerge(
      Layer.succeed(WorkflowAgentPort, {
        ensureStarted: () => Effect.succeed({ messageId: "message-stub" as never }),
        confirmStep: () => Effect.void,
        getDispatchForStep: () => Effect.succeed(null),
        awaitTerminal: () => Effect.succeed({ ok: true }),
        awaitStepTerminal: () => Effect.succeed({ ok: true }),
    ...
    Layer.provideMerge(
      Layer.succeed(WorkflowAgentPort, {
        ...
        cleanupSession: (threadId) =>
          Effect.sync(() => {
            providerSessionCalls.push(`cleanup:${threadId as string}`);
          }),
        recoverPending: () => Effect.void,
      }),
    ),
...
terminalTimeoutLayer("RealStepExecutor terminal-wait timeout", (it) => {
  it.effect("stops the provider session when the turn never reached a terminal state", () =>
    Effect.gen(function* () {
      ...
      // The still-live agent must be cleaned up so it cannot keep mutating the
      // worktree after the pipeline routed on.
      const threadId = (timeoutDispatchInputs[0] as { readonly threadId: string }).threadId;
      assert.deepEqual(providerSessionCalls, [`cleanup:${threadId}`]);
    }),
  );
});
...
const continueSessionLayer = (terminal: WorkflowDispatchTerminalResult) =>
  it.layer(
    RealStepExecutorLive.pipe(
      ...
      Layer.provideMerge(
        Layer.succeed(WorkflowAgentPort, {
          ...
          cleanupSession: (threadId) =>
            Effect.sync(() => {
              continueProviderCalls.push(`cleanup:${threadId as string}`);
            }),
          recoverPending: () => Effect.void,
        }),
      ),
...
continueSessionLayer({ ok: false, error: "turn did not reach a terminal state before timeout" })(
  ...
  (it) => {
    it.effect("still cleans up the session on failure, then resumes next run", () =>
      Effect.gen(function* () {
        continueDispatchInputs.length = 0;
        continueUpsertCalls.length = 0;
        ...
        // Failure cleanup MUST still run, while the resume cursor stays stored.
        assert.deepEqual(continueProviderCalls, [`cleanup:${failedThread}`]);
