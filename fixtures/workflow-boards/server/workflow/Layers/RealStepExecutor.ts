...
import type { StepOutcome, WorkflowStepUsage } from "../../../contracts/workflow.ts";
...
import { WorktreePort, type WorktreeHandle } from "../Services/WorktreePort.ts";
import {
  containsRealPath,
...
const make = Effect.gen(function* () {
  ...
  const runReviewPanel = (
    ...
    runTurn: (
      ...
      {
        readonly terminal: WorkflowDispatchTerminalResult;
    ...
    Effect.gen(function* () {
      ...
      for (const [index, member] of members.entries()) {
        ...
        const output = yield* agentPort.readCapturedOutput({
      ...
      for (const member of members) {
        ...
        // cleanupSession already swallows expected provider shutdown failures,
        // so this is a defensive best-effort guard; `Effect.ignore`
        // discards any typed failure while letting genuine defects surface.
        yield* agentPort.cleanupSession(member.threadId as ThreadId).pipe(Effect.ignore);
      }
      yield* agentPort.confirmStep(ctx.stepRunId).pipe(Effect.catch(() => Effect.void));
  ...
  const executeAgentStep = (
    ...
    Effect.gen(function* () {
      ...
      // Per-provider budgets are deferred; use the fork's default budget.
      const providerBudget = providerInputBudget(undefined);
      ...
      const runTurn = (
        ...
        Effect.gen(function* () {
          const started = yield* agentPort.ensureStarted({
            ...
            ...(worktree.projectId === undefined
              ? {}
              : { projectId: worktree.projectId as ProjectId }),
            threadTitle: `Workflow step ${step.key}${titleSuffix} - ${ctx.ticketId}`,
          });
          const terminal = yield* agentPort.awaitTerminal(
            turnIds.dispatchId as never,
            turnIds.threadId as never,
          );
          return { terminal, messageId: started.messageId, threadId: turnIds.threadId };
        });
      ...
      // is a harmless no-op.
      yield* agentPort.cleanupSession(result.threadId as ThreadId);
