...
const make = Effect.gen(function* () {
  ...
  const build: WorkflowRoutingContextBuilderShape["build"] = (input) =>
    Effect.gen(function* () {
      ...
      // A retried step has one p_workflow_boards_projection_step_run row per
      // attempt sharing the same stepKey. listStepRunsForPipeline orders by
      // started_at ASC, rowid ASC, so fromEntries deliberately keeps the LAST
      // (most recent) attempt: routing predicates evaluate against the final
      // attempt's outcome, not any earlier failed attempt. Predicates cannot
      // observe prior attempts.
