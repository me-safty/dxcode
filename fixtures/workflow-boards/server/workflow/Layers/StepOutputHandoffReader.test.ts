import { LaneKey, PipelineRunId, StepKey, TicketId } from "../../../contracts/workflow.ts";
...
const readerLayer = it.layer(StepOutputHandoffReaderLive.pipe(Layer.provideMerge(TestSql)));
...
const seedStepRun = (input: {
  ...
  Effect.gen(function* () {
    const sql = yield* SqlClient.SqlClient;
    const outputJson = input.output === undefined ? null : yield* encodeUnknownJson(input.output);
    yield* sql`
...
readerLayer("StepOutputHandoffReader", (it) => {
  ...
  it.effect("currentPassOutput returns this pass's output for the step key", () =>
    Effect.gen(function* () {
      ...
      // Same step key in a different pipeline run is not returned by currentPassOutput.
      yield* seedStepRun({
