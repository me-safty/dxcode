...
layer("WorkflowEngine work_source unlocked ops", (it) => {
  ...
  it.effect("closeTicketFromSourceUnlocked supersedes running work via the external path", () =>
    Effect.gen(function* () {
      ...
      // Seed a projected dispatch outbox row standing in for in-flight work. The
      // external supersession path (which closeTicketFromSourceUnlocked reuses)
      // tombstones A2-active rows to 'abandoned' so restart recovery never
      ...
      yield* sql`
        INSERT INTO p_workflow_boards_dispatch_outbox (
          ...
          'sonnet', 'do it', '/tmp/wt', 'projected', '2026-06-13T00:00:00.000Z'
        )
      `;
