...
layer("WorkflowReadModel", (it) => {
  ...
  it.effect(
    ...
    () =>
      ...
      `;
        ...
        yield* sql`
        INSERT INTO p_workflow_boards_projection_ticket_dependency (ticket_id, depends_on_ticket_id)
      ...
      `;
        ...
        yield* sql`
        UPDATE p_workflow_boards_projection_ticket
  ...
  it.effect("lists a capped ticket discussion newest-last without decoding attachments", () =>
    Effect.gen(function* () {
      ...
      const sql = yield* SqlClient.SqlClient;
      yield* sql`
        INSERT INTO p_workflow_boards_projection_ticket_message (
          message_id,
          ticket_id,
          step_run_id,
          author,
  ...
  it.effect("deletes ticket-scoped projections for a board without deleting other boards", () =>
    Effect.gen(function* () {
      ...
      const remaining = yield* sql<{ readonly tableName: string; readonly count: number }>`
        ...
        UNION ALL
        SELECT 'p_workflow_boards_projection_pipeline_run' AS tableName, COUNT(*) AS count
        FROM p_workflow_boards_projection_pipeline_run
        WHERE ticket_id = 'ticket-cascade'
        UNION ALL
        SELECT 'p_workflow_boards_projection_step_run' AS tableName, COUNT(*) AS count
        FROM p_workflow_boards_projection_step_run
        WHERE ticket_id = 'ticket-cascade'
        UNION ALL
        SELECT 'p_workflow_boards_script_run' AS tableName, COUNT(*) AS count
        FROM p_workflow_boards_script_run
        WHERE ticket_id = 'ticket-cascade'
        UNION ALL
        SELECT 'p_workflow_boards_dispatch_outbox' AS tableName, COUNT(*) AS count
        FROM p_workflow_boards_dispatch_outbox
        WHERE ticket_id = 'ticket-cascade'
        UNION ALL
        SELECT 'p_workflow_boards_setup_run' AS tableName, COUNT(*) AS count
        FROM p_workflow_boards_setup_run
        WHERE ticket_id = 'ticket-cascade'
        UNION ALL
        SELECT 'p_workflow_boards_projection_ticket_message' AS tableName, COUNT(*) AS count
        FROM p_workflow_boards_projection_ticket_message
        WHERE ticket_id = 'ticket-cascade'
      `;
      assert.deepEqual(
        ...
        [
          ...
          ["p_workflow_boards_projection_pipeline_run", 0],
          ["p_workflow_boards_projection_step_run", 0],
          ["p_workflow_boards_script_run", 0],
          ["p_workflow_boards_dispatch_outbox", 0],
          ["p_workflow_boards_setup_run", 0],
          ["p_workflow_boards_projection_ticket_message", 0],
        ],
      );
      ...
      assert.deepEqual(
        ...
        [
          ["p_workflow_boards_projection_ticket", 1],
          ["p_workflow_boards_projection_pipeline_run", 1],
          ["p_workflow_boards_projection_step_run", 1],
          ["p_workflow_boards_script_run", 1],
          ["p_workflow_boards_dispatch_outbox", 1],
          ["p_workflow_boards_setup_run", 1],
          ["p_workflow_boards_projection_ticket_message", 1],
        ],
      );
    }),
  );
  ...
  it.effect(
    ...
    () =>
      ...
      `;
        ...
        SELECT 'p_workflow_boards_projection_ticket' AS tableName,
        ...
        FROM p_workflow_boards_projection_ticket
        UNION ALL
        SELECT 'p_workflow_boards_projection_pipeline_run' AS tableName,
          SUM(CASE WHEN ticket_id = 'ticket-delete-one' THEN 1 ELSE 0 END) AS deleted,
          SUM(CASE WHEN ticket_id = 'ticket-keep-one' THEN 1 ELSE 0 END) AS kept
        FROM p_workflow_boards_projection_pipeline_run
        UNION ALL
        SELECT 'p_workflow_boards_projection_step_run' AS tableName,
          SUM(CASE WHEN ticket_id = 'ticket-delete-one' THEN 1 ELSE 0 END) AS deleted,
          SUM(CASE WHEN ticket_id = 'ticket-keep-one' THEN 1 ELSE 0 END) AS kept
        FROM p_workflow_boards_projection_step_run
        UNION ALL
        SELECT 'p_workflow_boards_script_run' AS tableName,
          SUM(CASE WHEN ticket_id = 'ticket-delete-one' THEN 1 ELSE 0 END) AS deleted,
          SUM(CASE WHEN ticket_id = 'ticket-keep-one' THEN 1 ELSE 0 END) AS kept
        FROM p_workflow_boards_script_run
        UNION ALL
        SELECT 'p_workflow_boards_dispatch_outbox' AS tableName,
          SUM(CASE WHEN ticket_id = 'ticket-delete-one' THEN 1 ELSE 0 END) AS deleted,
          SUM(CASE WHEN ticket_id = 'ticket-keep-one' THEN 1 ELSE 0 END) AS kept
        FROM p_workflow_boards_dispatch_outbox
        UNION ALL
        SELECT 'p_workflow_boards_setup_run' AS tableName,
          SUM(CASE WHEN ticket_id = 'ticket-delete-one' THEN 1 ELSE 0 END) AS deleted,
          SUM(CASE WHEN ticket_id = 'ticket-keep-one' THEN 1 ELSE 0 END) AS kept
        FROM p_workflow_boards_setup_run
        UNION ALL
        SELECT 'p_workflow_boards_projection_ticket_message' AS tableName,
          SUM(CASE WHEN ticket_id = 'ticket-delete-one' THEN 1 ELSE 0 END) AS deleted,
          SUM(CASE WHEN ticket_id = 'ticket-keep-one' THEN 1 ELSE 0 END) AS kept
        FROM p_workflow_boards_projection_ticket_message
      `;
        ...
        assert.deepEqual(
          ...
          [
            ["p_workflow_boards_projection_ticket", 0, 1],
            ["p_workflow_boards_projection_pipeline_run", 0, 1],
            ["p_workflow_boards_projection_step_run", 0, 1],
            ["p_workflow_boards_script_run", 0, 1],
            ["p_workflow_boards_dispatch_outbox", 0, 1],
            ["p_workflow_boards_setup_run", 0, 1],
            ["p_workflow_boards_projection_ticket_message", 0, 1],
          ],
        );
      }),
  );
  ...
  it.effect(
    "listTickets and getTicketDetail include pr field when p_workflow_boards_pr_state row exists",
    () =>
      Effect.gen(function* () {
        const read = yield* WorkflowReadModel;
        const sql = yield* SqlClient.SqlClient;
  ...
  it.effect(
    "deleteBoardTicketState removes p_workflow_boards_work_source_mapping and p_workflow_boards_work_source_state rows for the board",
    () =>
      Effect.gen(function* () {
        const read = yield* WorkflowReadModel;
        const sql = yield* SqlClient.SqlClient;
        ...
        // Insert p_workflow_boards_work_source_mapping row
        yield* sql`
          INSERT INTO p_workflow_boards_work_source_mapping (
        ...
        // Insert p_workflow_boards_work_source_state row (board-scoped)
        yield* sql`
          INSERT INTO p_workflow_boards_work_source_state (
        ...
        const mappingCount = yield* sql<{ readonly count: number }>`
          SELECT COUNT(*) AS count FROM p_workflow_boards_work_source_mapping WHERE ticket_id = 'ticket-ws-cascade'
        `;
        assert.equal(
          mappingCount[0]?.count,
          0,
          "p_workflow_boards_work_source_mapping should be deleted",
        );

        const stateCount = yield* sql<{ readonly count: number }>`
          SELECT COUNT(*) AS count FROM p_workflow_boards_work_source_state WHERE board_id = 'b-ws-cascade'
        `;
        assert.equal(
          stateCount[0]?.count,
          0,
          "p_workflow_boards_work_source_state should be deleted",
        );
      }),
  );

  it.effect("board deletion cascades p_workflow_boards_outbound_delivery rows", () =>
    Effect.gen(function* () {
      const read = yield* WorkflowReadModel;
      const sql = yield* SqlClient.SqlClient;
      const now = "2026-06-13T00:00:00.000Z";
      ...
      yield* sql`
        INSERT INTO p_workflow_boards_outbound_connection (
      ...
      yield* sql`
        INSERT INTO p_workflow_boards_outbound_delivery (
      ...
      const deliveryCount = yield* sql<{ readonly count: number }>`
        SELECT COUNT(*) AS count FROM p_workflow_boards_outbound_delivery WHERE board_id = 'b1'
      `;
      assert.equal(
        deliveryCount[0]?.count,
        0,
        "p_workflow_boards_outbound_delivery should be deleted",
      );
      ...
      const connectionCount = yield* sql<{ readonly count: number }>`
        SELECT COUNT(*) AS count FROM p_workflow_boards_outbound_connection WHERE connection_ref = 'conn-keep'
      `;
      assert.equal(
        connectionCount[0]?.count,
        1,
        "p_workflow_boards_outbound_connection should NOT be cascaded by board deletion",
      );
    }),
  );

  it.effect(
    "deleteTicketState removes p_workflow_boards_work_source_mapping for the ticket but leaves board-scoped p_workflow_boards_work_source_state intact",
    () =>
      Effect.gen(function* () {
        const read = yield* WorkflowReadModel;
        const sql = yield* SqlClient.SqlClient;
        ...
        // Insert p_workflow_boards_work_source_mapping row for the ticket
        yield* sql`
          INSERT INTO p_workflow_boards_work_source_mapping (
            mapping_id, board_id, source_id, provider, external_id, ticket_id,
            content_hash, lifecycle, sync_status, created_at, last_synced_at
        ...
        // Insert board-scoped p_workflow_boards_work_source_state row (should NOT be deleted)
        yield* sql`
          INSERT INTO p_workflow_boards_work_source_state (
            board_id, source_id, consecutive_failures
          )
        ...
        const mappingCount = yield* sql<{ readonly count: number }>`
          SELECT COUNT(*) AS count FROM p_workflow_boards_work_source_mapping WHERE ticket_id = 'ticket-ws-single'
        `;
        assert.equal(
          mappingCount[0]?.count,
          0,
          "p_workflow_boards_work_source_mapping should be deleted for the ticket",
        );

        const stateCount = yield* sql<{ readonly count: number }>`
          SELECT COUNT(*) AS count FROM p_workflow_boards_work_source_state WHERE board_id = 'b-ws-ticket'
        `;
        assert.equal(
          stateCount[0]?.count,
          1,
          "p_workflow_boards_work_source_state (board-scoped) should remain untouched",
        );
      }),
  );

  it.effect(
    "getTicketDetail returns syncedSource when p_workflow_boards_work_source_mapping row has valid source_metadata_json",
    () =>
      Effect.gen(function* () {
        const read = yield* WorkflowReadModel;
        const sql = yield* SqlClient.SqlClient;
  ...
  it.effect("deleteBoardTicketState cascades p_workflow_boards_board_proposal rows", () =>
    Effect.gen(function* () {
      const read = yield* WorkflowReadModel;
      const sql = yield* SqlClient.SqlClient;
      ...
      yield* sql`
        INSERT INTO p_workflow_boards_board_proposal (
      ...
      const deletedCount = yield* sql<{ readonly count: number }>`
        SELECT COUNT(*) AS count FROM p_workflow_boards_board_proposal WHERE board_id = 'b-proposal-cascade'
      `;
      assert.equal(
        deletedCount[0]?.count,
        0,
        "p_workflow_boards_board_proposal rows for board should be deleted",
      );

      const keptCount = yield* sql<{ readonly count: number }>`
        SELECT COUNT(*) AS count FROM p_workflow_boards_board_proposal WHERE board_id = 'b-other'
      `;
      assert.equal(
        keptCount[0]?.count,
        1,
        "p_workflow_boards_board_proposal rows for other boards must NOT be cascaded",
      );
    }),
  );
