...
const make = Effect.gen(function* () {
  ...
  const upsert: WorkflowAgentSessionStoreShape["upsert"] = (
    ...
    Effect.gen(function* () {
      ...
      yield* wrap(
        ...
        sql`
          INSERT INTO p_workflow_boards_agent_session
  ...
  const getThreadId: WorkflowAgentSessionStoreShape["getThreadId"] = (
    ...
    wrap(
      ...
      sql<{ readonly threadId: string }>`
        ...
        FROM p_workflow_boards_agent_session
  ...
  const deleteByTicket: WorkflowAgentSessionStoreShape["deleteByTicket"] = (ticketId) =>
    wrap(
      ...
      sql`
        DELETE FROM p_workflow_boards_agent_session
        WHERE ticket_id = ${String(ticketId)}
      `,
    ).pipe(Effect.asVoid);

  const listByBoard: WorkflowAgentSessionStoreShape["listByBoard"] = (boardId: BoardId) =>
    wrap(
      ...
      sql<RawRow>`
        ...
        FROM p_workflow_boards_agent_session AS s
        JOIN p_workflow_boards_projection_ticket AS t ON t.ticket_id = s.ticket_id
  ...
  const deleteByBoard: WorkflowAgentSessionStoreShape["deleteByBoard"] = (boardId) =>
    wrap(
      ...
      sql`
        DELETE FROM p_workflow_boards_agent_session
        WHERE ticket_id IN (
          SELECT ticket_id FROM p_workflow_boards_projection_ticket WHERE board_id = ${String(boardId)}
        )
      `,
    ).pipe(Effect.asVoid);
