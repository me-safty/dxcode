...
interface TicketDependencySqlRow extends TicketRow {
  ...
  // PR columns from the p_workflow_boards_pr_state LEFT JOIN — null when no row exists.
  readonly prNumber?: number | null;
  readonly prUrl?: string | null;
  readonly prState?: string | null;
  readonly prCiState?: string | null;
  // Work-source columns from the p_workflow_boards_work_source_mapping LEFT JOIN — null when no mapping row exists.
...
/**
 * Parse `source_metadata_json` from a `p_workflow_boards_work_source_mapping` row into the
...
const make = Effect.gen(function* () {
  ...
  const deleteBoard: WorkflowReadModelShape["deleteBoard"] = (boardId) =>
    wrap(sql`
      DELETE FROM p_workflow_boards_projection_board
      WHERE board_id = ${boardId}
    `).pipe(Effect.asVoid);
  ...
  const deleteBoardTicketState: WorkflowReadModelShape["deleteBoardTicketState"] = (boardId) =>
    ...
    `).pipe(
      Effect.andThen(
        wrap(sql`
          DELETE FROM p_workflow_boards_setup_run
          WHERE ticket_id IN (
            SELECT ticket_id
            FROM p_workflow_boards_projection_ticket
            WHERE board_id = ${boardId}
      ...
      Effect.andThen(
        wrap(sql`
          DELETE FROM p_workflow_boards_script_run
          WHERE ticket_id IN (
            SELECT ticket_id
            FROM p_workflow_boards_projection_ticket
            WHERE board_id = ${boardId}
      ...
      Effect.andThen(
        wrap(sql`
          DELETE FROM p_workflow_boards_projection_step_run
          WHERE ticket_id IN (
            SELECT ticket_id
            FROM p_workflow_boards_projection_ticket
            WHERE board_id = ${boardId}
      ...
      Effect.andThen(
        wrap(sql`
          DELETE FROM p_workflow_boards_projection_pipeline_run
          WHERE ticket_id IN (
            SELECT ticket_id
            FROM p_workflow_boards_projection_ticket
            WHERE board_id = ${boardId}
      ...
      Effect.andThen(
        wrap(sql`
          DELETE FROM p_workflow_boards_projection_ticket_message
          WHERE ticket_id IN (
            SELECT ticket_id
            FROM p_workflow_boards_projection_ticket
            WHERE board_id = ${boardId}
      ...
      Effect.andThen(
        wrap(sql`
          DELETE FROM p_workflow_boards_projection_ticket_dependency
          WHERE ticket_id IN (
            SELECT ticket_id
            FROM p_workflow_boards_projection_ticket
            WHERE board_id = ${boardId}
      ...
      Effect.andThen(
        wrap(sql`
          DELETE FROM p_workflow_boards_pr_observation
          WHERE ticket_id IN (
            SELECT ticket_id
            FROM p_workflow_boards_projection_ticket
            WHERE board_id = ${boardId}
      ...
      Effect.andThen(
        wrap(sql`
          DELETE FROM p_workflow_boards_pr_state
          WHERE ticket_id IN (
            SELECT ticket_id
            FROM p_workflow_boards_projection_ticket
            WHERE board_id = ${boardId}
      ...
      Effect.andThen(
        wrap(sql`
          DELETE FROM p_workflow_boards_work_source_mapping
          WHERE ticket_id IN (
            SELECT ticket_id
            FROM p_workflow_boards_projection_ticket
            WHERE board_id = ${boardId}
      ...
      Effect.andThen(
        wrap(sql`
          DELETE FROM p_workflow_boards_work_source_state
          WHERE board_id = ${boardId}
        `),
      ),
      Effect.andThen(
        ...
        wrap(sql`
          DELETE FROM p_workflow_boards_outbound_delivery
          WHERE board_id = ${boardId}
        `),
      ),
      Effect.andThen(
        wrap(sql`
          DELETE FROM p_workflow_boards_board_proposal
          WHERE board_id = ${boardId}
        `),
      ),
      Effect.andThen(
        wrap(sql`
          DELETE FROM p_workflow_boards_projection_ticket
          WHERE board_id = ${boardId}
        `),
      ),
      Effect.asVoid,
  ...
  const listTickets: WorkflowReadModelShape["listTickets"] = (boardId) =>
    wrap(sql<TicketDependencySqlRow>`
      SELECT
        p_workflow_boards_projection_ticket.ticket_id AS "ticketId",
        board_id AS "boardId",
        title,
        description,
        ...
        p_workflow_boards_projection_ticket.updated_at AS "updatedAt",
        (
          SELECT SUM(step.total_tokens)
          FROM p_workflow_boards_projection_step_run AS step
          WHERE step.ticket_id = p_workflow_boards_projection_ticket.ticket_id
        ) AS "totalTokens",
        (
          SELECT CAST(
        ...
        (
          SELECT COUNT(*)
          FROM p_workflow_boards_projection_ticket_dependency AS dep
          LEFT JOIN p_workflow_boards_projection_ticket AS dep_ticket
            ON dep_ticket.ticket_id = dep.depends_on_ticket_id
          WHERE dep.ticket_id = p_workflow_boards_projection_ticket.ticket_id
        ...
        status
      FROM p_workflow_boards_projection_ticket
      LEFT JOIN p_workflow_boards_pr_state AS pr
        ON pr.ticket_id = p_workflow_boards_projection_ticket.ticket_id
      WHERE board_id = ${boardId}
    ...
    wrap(sql<TicketRow>`
      ...
      WHERE board_id = ${boardId}
        ...
        AND NOT EXISTS (
          SELECT 1
          FROM p_workflow_boards_projection_ticket_dependency AS dep
          INNER JOIN p_workflow_boards_projection_ticket AS dep_ticket
            ON dep_ticket.ticket_id = dep.depends_on_ticket_id
          WHERE dep.ticket_id = p_workflow_boards_projection_ticket.ticket_id
            AND dep_ticket.terminal_at IS NULL
        )
  ...
  const getTicketDetail: WorkflowReadModelShape["getTicketDetail"] = (ticketId) =>
    Effect.gen(function* () {
      const ticketRows = yield* wrap(sql<TicketDependencySqlRow>`
        SELECT
          p_workflow_boards_projection_ticket.ticket_id AS "ticketId",
          p_workflow_boards_projection_ticket.board_id AS "boardId",
          title,
          description,
          current_lane_key AS "currentLaneKey",
          current_lane_entry_token AS "currentLaneEntryToken",
        ...
        LEFT JOIN p_workflow_boards_pr_state AS pr
          ON pr.ticket_id = p_workflow_boards_projection_ticket.ticket_id
        LEFT JOIN p_workflow_boards_work_source_mapping AS wsm
          ON wsm.ticket_id = p_workflow_boards_projection_ticket.ticket_id
        WHERE p_workflow_boards_projection_ticket.ticket_id = ${ticketId}
      `);
      ...
      const stepRows = yield* wrap(sql<StepRunSqlRow>`
        ...
        FROM p_workflow_boards_projection_step_run AS step
        LEFT JOIN p_workflow_boards_script_run AS script
  ...
  const listReleasableDependents: WorkflowReadModelShape["listReleasableDependents"] = (ticketId) =>
    wrap(sql<{ readonly ticketId: string; readonly boardId: string; readonly laneKey: string }>`
      ...
      FROM p_workflow_boards_projection_ticket_dependency AS dep
      INNER JOIN p_workflow_boards_projection_ticket AS dependent
        ON dependent.ticket_id = dep.ticket_id
      WHERE dep.depends_on_ticket_id = ${ticketId}
        AND dependent.queued_at IS NOT NULL
        AND NOT EXISTS (
          SELECT 1
          FROM p_workflow_boards_projection_ticket_dependency AS other
          INNER JOIN p_workflow_boards_projection_ticket AS other_ticket
  ...
  const listDependentTicketIds: WorkflowReadModelShape["listDependentTicketIds"] = (ticketId) =>
    wrap(sql<{ readonly ticketId: string }>`
      ...
      FROM p_workflow_boards_projection_ticket_dependency
  ...
  const getBoardDigest: WorkflowReadModelShape["getBoardDigest"] = (boardId, windowHours) =>
    Effect.gen(function* () {
      ...
      const usage = yield* wrap(sql<{
        ...
        FROM p_workflow_boards_projection_step_run AS step
        INNER JOIN p_workflow_boards_projection_ticket AS ticket ON ticket.ticket_id = step.ticket_id
    ...
    Effect.gen(function* () {
      ...
      const routeRows = yield* wrap(sql<{
        ...
        WHERE event_type = 'TicketRouteDecided'
          ...
          AND ticket_id IN (SELECT ticket_id FROM p_workflow_boards_projection_ticket WHERE board_id = ${boardId})
      ...
      const stepRows = yield* wrap(sql<{
        ...
        FROM p_workflow_boards_projection_step_run AS sr
        INNER JOIN p_workflow_boards_projection_pipeline_run AS pr ON pr.pipeline_run_id = sr.pipeline_run_id
        WHERE sr.ticket_id IN (SELECT ticket_id FROM p_workflow_boards_projection_ticket WHERE board_id = ${boardId})
  ...
  // path (p_workflow_boards_projection_board carries project_id, not an environment_id, and the
  ...
  const listNeedsAttentionTickets: WorkflowReadModelShape["listNeedsAttentionTickets"] = () =>
    wrap(sql<WorkflowNeedsAttentionTicketRow>`
      ...
      FROM p_workflow_boards_projection_ticket AS pt
      INNER JOIN p_workflow_boards_projection_board AS pb
  ...
  const countLanePipelineRuns: WorkflowReadModelShape["countLanePipelineRuns"] = (pipelineRunId) =>
    wrap(sql<{ readonly count: number }>`
      SELECT COUNT(*) AS count
      FROM p_workflow_boards_events AS started
      INNER JOIN p_workflow_boards_projection_pipeline_run AS current
      ...
      WHERE started.ticket_id = current.ticket_id
        ...
        AND started.sequence > COALESCE(
          (
            ...
            FROM p_workflow_boards_events AS reset
  ...
  const listBoardProposals: WorkflowReadModelShape["listBoardProposals"] = (boardId) =>
    Effect.gen(function* () {
      ...
      const rows = yield* wrap(sql<ProposalSqlRow>`
        ...
        FROM p_workflow_boards_board_proposal
        WHERE board_id = ${boardId}
        ORDER BY
  ...
  const listLiveOccupiedLanes: WorkflowReadModelShape["listLiveOccupiedLanes"] = (boardId) =>
    wrap(sql<{ readonly laneKey: string }>`
      ...
      FROM p_workflow_boards_projection_pipeline_run AS run
      INNER JOIN p_workflow_boards_projection_ticket AS t ON t.ticket_id = run.ticket_id
    ...
    sql
      .withTransaction(
        sql<{ readonly proposalId: string }>`
          UPDATE p_workflow_boards_board_proposal
          SET
            status = ${input.status},
  ...
  const listWorkSourceMappingsForBoard: WorkflowReadModelShape["listWorkSourceMappingsForBoard"] = (
    ...
    wrap(sql<WorkSourceMappingRow>`
      ...
      FROM p_workflow_boards_work_source_mapping m
      JOIN p_workflow_boards_projection_ticket t ON t.ticket_id = m.ticket_id
