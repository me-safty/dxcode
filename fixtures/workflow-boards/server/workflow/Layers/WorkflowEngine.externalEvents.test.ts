...
layer("WorkflowEngine pr.* predicate context", (it) => {
  it.effect("pr.approved with pr.ciState=success moves the ticket; with pending stays noop", () =>
    Effect.gen(function* () {
      ...
      // Seed p_workflow_boards_pr_state with last_ci_state='success'
      yield* sql`
  ...
  it.effect("pr.approved with pr.ciState=pending stays noop", () =>
    Effect.gen(function* () {
      ...
      // Seed p_workflow_boards_pr_state with last_ci_state='pending'
      yield* sql`
        INSERT INTO p_workflow_boards_pr_state (
          ticket_id, pr_number, pr_url, branch, remote_name, repo,
          pr_state, last_ci_state, last_review_decision, updated_at
  ...
  it.effect("pr.approved with no p_workflow_boards_pr_state row stays noop", () =>
    Effect.gen(function* () {
      const registry = yield* BoardRegistry;
      ...
      // No p_workflow_boards_pr_state row → pr.ciState is null → predicate fails
      const noop = yield* engine.ingestExternalEvent({
        boardId: "b-pr-ci-norow" as never,
        name: "pr.approved",
        ticketId,
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        // Seed p_workflow_boards_pr_state with last_review_decision='approved'
        yield* sql`
          INSERT INTO p_workflow_boards_pr_state (
            ticket_id, pr_number, pr_url, branch, remote_name, repo,
            pr_state, last_ci_state, last_review_decision, updated_at
  ...
  it.effect("ci.passed with pr.reviewDecision=none stays noop", () =>
    Effect.gen(function* () {
      ...
      // Seed p_workflow_boards_pr_state with last_review_decision='none'
      yield* sql`
        INSERT INTO p_workflow_boards_pr_state (
          ticket_id, pr_number, pr_url, branch, remote_name, repo,
          pr_state, last_ci_state, last_review_decision, updated_at
  ...
  it.effect("flows pr context (ciState + reviewDecision) through to predicate evaluation", () =>
    Effect.gen(function* () {
      ...
      // The engine reads p_workflow_boards_pr_state once and exposes pr.ciState /
