...
layer("WorkflowProjectionPipeline", (it) => {
  it.effect(
    "projects TicketCreated then TicketMovedToLane into p_workflow_boards_projection_ticket",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("projects step lifecycle and waiting_on_user status", () =>
    Effect.gen(function* () {
      ...
      const ticket = yield* sql<{ readonly status: string }>`
        SELECT status FROM p_workflow_boards_projection_ticket WHERE ticket_id = 't-2'
      `;
  ...
  it.effect("projects TicketPrOpened into p_workflow_boards_pr_state (initial insert)", () =>
    Effect.gen(function* () {
      const pipeline = yield* WorkflowProjectionPipeline;
      const sql = yield* SqlClient.SqlClient;
  ...
  it.effect("projecting TicketPrOpened twice is idempotent (upsert)", () =>
    Effect.gen(function* () {
      ...
      const rows = yield* sql<{ readonly count: number }>`
        SELECT COUNT(*) AS count FROM p_workflow_boards_pr_state WHERE ticket_id = 'ticket-pr-replay'
      `;
      assert.equal(rows[0]?.count, 1);
    }),
  );

  it.effect("projects script step start and exit into p_workflow_boards_script_run", () =>
    Effect.gen(function* () {
      const pipeline = yield* WorkflowProjectionPipeline;
      const sql = yield* SqlClient.SqlClient;
      const base = {
