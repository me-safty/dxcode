...
import type { BoardTicketView } from "../../../contracts/workflow.ts";
...
const countMappings = (boardId: string) =>
  Effect.gen(function* () {
    ...
    const rows = yield* sql<{ readonly count: number }>`
      SELECT COUNT(*) AS count FROM p_workflow_boards_work_source_mapping WHERE board_id = ${boardId}
    `;
...
layer("WorkflowSourceCommitter.reconcileChunk", (it) => {
  ...
  it.effect("idempotent create: the same new delta twice yields exactly one ticket + mapping", () =>
    Effect.gen(function* () {
      ...
      const tickets = yield* sql<{ readonly count: number }>`
        SELECT COUNT(*) AS count FROM p_workflow_boards_projection_ticket WHERE board_id = 'b-idem'
      `;
  ...
  it.effect(
    ...
    () =>
      Effect.gen(function* () {
        ...
        SELECT COUNT(*) AS count FROM p_workflow_boards_projection_pipeline_run
  ...
  it.effect("validate lanes: a missing destination lane fails the chunk; nothing is created", () =>
    Effect.gen(function* () {
      ...
      const tickets = yield* sql<{ readonly count: number }>`
        SELECT COUNT(*) AS count FROM p_workflow_boards_projection_ticket WHERE board_id = 'b-badlane'
      `;
  ...
  it.effect("validate lanes: a missing closed lane fails the chunk; nothing is created", () =>
    Effect.gen(function* () {
      ...
      const tickets = yield* sql<{ readonly count: number }>`
        SELECT COUNT(*) AS count FROM p_workflow_boards_projection_ticket WHERE board_id = 'b-badclosed'
      `;
      assert.equal(tickets[0]?.count ?? 0, 0);
    }),
  );
...
// ---------------------------------------------------------------------------
// Fixes 2 (post-tx provider cleanup), 3 (no orphan on UNIQUE), 6 (post-tx
// publish). Each uses a fresh layer so DeterministicWorkflowIds counters start
// clean (Fix 3 predicts the first ticket id) and a recording WorkflowAgentPort /
      ...
      Layer.succeed(WorkflowAgentPort, {
        ...
        cleanupSession: (threadId) =>
          Ref.update(providerCalls, (calls) => [
            ...calls,
            { kind: "cleanup" as const, threadId: threadId as string },
          ]),
...
it.effect(
  "Fix 2: a closed delta with running provider work cleans up the session POST-commit",
  () =>
    Effect.gen(function* () {
      ...
      `;
        ...
        // The provider session was cleaned up post-commit.
        const calls = yield* Ref.get(providerCalls);
        assert.deepEqual(calls, [{ kind: "cleanup", threadId: "thread-cancel-1" }]);

        // The dispatch row was tombstoned in-tx (no live A2 active rows).
        const live = yield* sql<{ readonly count: number }>`
        SELECT COUNT(*) AS count FROM p_workflow_boards_dispatch_outbox
        WHERE ticket_id = ${ticketId} AND status IN ('reserved', 'start_requested', 'projected')
      `;
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        INSERT INTO p_workflow_boards_dispatch_outbox (
          ...
          'message-rollback-1', 'codex', 'gpt-5.5', 'cancel me', '/tmp/wt', 'projected',
          '2026-06-13T00:00:00.000Z', '2026-06-13T00:00:00.000Z'
        )
      `;
        ...
        FROM p_workflow_boards_projection_ticket WHERE ticket_id LIKE 'ticket-%'
  ...
  () =>
    Effect.gen(function* () {
      ...
      `;
        ...
        const tickets = yield* sql<{ readonly count: number }>`
        SELECT COUNT(*) AS count FROM p_workflow_boards_projection_ticket WHERE board_id = 'b-orphan-guard'
      `;
        assert.equal(tickets[0]?.count ?? 0, 0);
      ...
      yield* Effect.gen(function* () {
        ...
        yield* sql`
          INSERT INTO p_workflow_boards_dispatch_outbox (
            ...
            'message-recover-fail-1', 'codex', 'gpt-5.5', 'cancel me', '/tmp/wt', 'projected',
            '2026-06-13T00:00:00.000Z', '2026-06-13T00:00:00.000Z'
          )
        `;
        ...
        // Provider cleanup STILL ran (independent of recoverBoardWip).
        assert.deepEqual(yield* Ref.get(providerCalls), [
          { kind: "cleanup", threadId: "thread-recover-fail-1" },
        ]);
