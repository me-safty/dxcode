...
// the two extra p_workflow_boards_projection_ticket point-reads.
...
const make = Effect.gen(function* () {
  ...
  const appendAndProjectUnlocked = (event: CommitEvent) =>
    Effect.gen(function* () {
      ...
      // two p_workflow_boards_projection_ticket point-reads and the insert(s) entirely.
      ...
      const prevRows = yield* sql<{
        ...
        FROM p_workflow_boards_projection_ticket WHERE ticket_id = ${event.ticketId}
      `;
