...
import {
  ...
  stackedThreadToast,
  toastManager,
  type PluginRouteComponentProps,
  type PluginWebRpc,
} from "@t3tools/plugin-sdk-web";
import type { WorkflowEnvironmentApi as EnvironmentApi } from "./workflowApi";
import {
  BoardId,
  LaneKey,
  StepRunId,
...
import { emptyBoardState, type BoardState } from "./boardState";
import {
  answerTicketStep,
  createTicket,
  editTicket,
...
const RIGHT_PANEL_SHEET_CLASS_NAME =
...
export interface BoardRouteProps extends PluginRouteComponentProps {
...
export function BoardRoute(props: BoardRouteProps) {
  const { rpc, api } = props;
  const rawEnvironmentId = props.environmentId;
  const { boardId: rawBoardId, ticket: rawTicket } = parseBoardRouteSearch(props.search);
  ...
  const environmentId = useMemo(
    () => (rawEnvironmentId ? EnvironmentId.make(rawEnvironmentId) : null),
    [rawEnvironmentId],
  );
  ...
  // Only `workflow` is populated: the plugin RPC bridge exposes the `workflow.*`
  // methods, but NOT host `orchestration`/`terminal` RPCs. So the live "Agent
  // activity" feed, the agent-session dialog, and the script-output viewer render
  // empty (their `if (!api?.orchestration)` / `if (!api?.terminal)` guards hold).
  // Restoring them needs the SDK to expose orchestration.subscribeThread +
  // terminal.attachHistory — tracked as a follow-up.
  const routeApi = useMemo(() => ({ workflow: api }) as EnvironmentApi, [api]);

  // Board state from the folded subscription atom.
  const boardQuery = useBoardState(rpc, boardId);
  const state = AsyncResult.isSuccess(boardQuery) ? boardQuery.value : emptyBoardState;
  // Intentionally undefined: the host derived a ticket cwd from the board's project
  // workspaceRoot (for resolving relative file paths in ticket markdown). The plugin
  // does not yet resolve the project workspace, so relative paths render unresolved.
  const ticketCwd: string | undefined = undefined;
  ...
  useEffect(() => {
    ...
    return subscribeBoard(routeApi, boardId, {
      onSnapshot: (snapshot) => {
