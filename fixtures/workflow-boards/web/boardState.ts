// Pure board-stream reducer. Ported verbatim from the host's former
// client-runtime `state/boardState.ts` (the host no longer carries workflow
// client state — it moved into this plugin). Folds a `BoardStreamItem` stream
// (`workflow.subscribeBoard`) into a render-ready `BoardState`. See
// ./useBoardState.ts for the atom/hook that drives it off the plugin RPC bridge.
