...
- Generated from `upstream/main` `ecb35f75839925dd1ac6f854efeef5c9e291d11b` with starting branch HEAD `41b4d6c33afc15c0447d77bc4a3a443a70470f32`. The merge is intentionally left uncommitted with `MERGE_HEAD` at `ecb35f75839925dd1ac6f854efeef5c9e291d11b`; the previous committed upstream merge remains `2ce693ca5809a4561feb83f636d76d582ea13bf8`, whose upstream parent is `c1ec1915fc16f3dc1ec5d47d9a97f6210a574526`.
- Shared refs at merge time: `origin/main` `e5490591393654b39ac99e97ccae679f7157a765`, local `main` `e1495fd2f8ccc6c26ae4e34fafeae70ab11e7817`, and branch tracking ref `origin/split/version-control-panel-work` `41b4d6c33afc15c0447d77bc4a3a443a70470f32`.
- With the no-commit merge open, recorded HEAD is `77 ahead / 9 behind upstream/main`, `80 ahead / 465 behind origin/main`, and `77 ahead / 474 behind local main`. Committing the current merge result would make the branch `78 ahead / 0 behind upstream/main`, `90 ahead / 465 behind origin/main`, `87 ahead / 474 behind local main`, and `10 ahead / 0 behind origin/split/version-control-panel-work`. Including the staged upstream merge and this documentation refresh, the working-tree fork diff is `56 files changed, 15244 insertions(+), 353 deletions(-)` against the `upstream/main` tree.
- Merge note: the nine incoming commits from `8de254a685f4356c719e744926397362c800f426` through `ecb35f75839925dd1ac6f854efeef5c9e291d11b` add client-owned project-favicon fallbacks, stale working-task notification suppression, Android beta branding and review-diff/Git-sheet polish, lower-cost web status animations and grain rendering, corrected repository documentation links and CI commands, and marketing/mobile legal-document surfaces with a follow-up legal modal header fix. Git merged all files automatically with no conflicts, and the incoming and fork-only file sets did not overlap. The mobile Git sheets remain a separate client surface and do not replace the fork's singleton web Version Control panel, server-owned source-control operations, or panel RPC/contracts. The earlier merge's responsive thread error banner, offline environment/VCS-ref cache, worktree-aware `expectedBranch` compare-and-set updates, and VCS action presentation cleanup remain integrated with the panel's source-control-only errors, query/command atoms, monotonic metadata routing, and bounded snapshot/detail cache. No Version Control panel customization was changed or retired, upstream still does not provide an equivalent panel, and the incoming code did not expose a worthwhile consistency refactor for the panel.
