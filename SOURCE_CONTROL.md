...
- Generated from `upstream/main` `f61fa9499d96fee825492aba204593c37b27e0cb` with starting branch HEAD `d6f86b1fbf3028e0b44f104d66af85872fe38eeb`; the merge created local merge commit `37aa959a3ed4b32e42a59828aaf09936e68353da`.
- Shared refs at merge time: `origin/main` `4e49fb7be3d0349ecad4df23ce08a312609f21b5`, local `main` `1aa9302d43c711ad5ec049aabbd394df92aa09ac`.
- Effective branch delta after the merge and this snapshot refresh: `74 ahead / 0 behind upstream/main`, `100 ahead / 443 behind origin/main`, `100 ahead / 444 behind local main`; including the uncommitted merge adaptations and this documentation refresh, the current working-tree fork diff size is `56 files changed, 15240 insertions(+), 350 deletions(-)` against the `upstream/main` tree.
- Merge note: the 26 incoming commits from `6009720844f053e048e18ca1abe4f429559ab9fc` through `f61fa9499d96fee825492aba204593c37b27e0cb` added HTTP shell/thread snapshots with replay-safe WebSocket catch-up, offline environment and VCS-ref caches, T3 Connect onboarding, worktree-aware branch metadata compare-and-set updates, VCS action presentation cleanup, APNs/live-activity routing and delivery hardening, broad mobile persistence/UX/accessibility fixes, pnpm 11 and Clerk upgrades, Cursor Cloud setup, and desktop/mobile packaging changes. Git auto-merged the only two branch/upstream file overlaps, `apps/server/src/ws.ts` and `packages/client-runtime/src/state/vcs.ts`, without conflict markers: the result keeps upstream's HTTP catch-up and offline cached-ref architecture alongside the fork's source-control RPC authorization/handlers and panel query/command atoms. Focused tests exposed and the resolution restored the panel's `createEnvironmentRpcQueryAtomFamily` import after upstream stopped using it for cached `listRefs`. The follow-up adaptation in `apps/web/src/components/ChatView.sourceControl.ts`, its test, and `apps/web/src/components/ChatView.tsx` sends the active thread branch as upstream's new `expectedBranch` guard while retaining the fork's monotonic hook-lifetime sequencing and source-control-specific error ownership. Upstream's finished-action presentation reset remains complementary to the panel and does not replace its bounded snapshot/detail cache. The merge preserved the singleton panel, live watcher, Actionable/Remotes models, selected-file operations, branch/commit/stash/remote actions, compare semantics, provider/error handling, worktree-aware execution, refresh/cache safeguards, parsing hardening, and all other behavior documented below. No Version Control panel customization was retired, and upstream still does not provide an equivalent panel.
