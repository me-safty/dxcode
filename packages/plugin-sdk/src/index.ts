...
export interface ProjectionsReadCapability {
   ...
   */
  readonly getThreadShellById: (threadId: ThreadId) => Effect.Effect<OrchestrationThreadShell | null, Error>;

  /**
   * Read a single active thread detail snapshot by id. The lookup is
   * intentionally id-keyed and not owner-filtered.
   */
  readonly getThreadDetailById: (threadId: ThreadId) => Effect.Effect<OrchestrationThread | null, Error>;
...
export interface SourceControlCapability {
   ...
   */
  readonly discoverProviders: Effect.Effect<ReadonlyArray<SourceControlProviderDiscoveryItem>, Error>;
   ...
   */
  readonly getDefaultBranch: (input: { readonly cwd: string }) => Effect.Effect<string | null, Error>;
...
export interface PluginHttpResponse {
  ...
  readonly headers?: Readonly<Record<string, string>> | undefined;
  readonly body?: string | Uint8Array | ReadonlyArray<unknown> | Readonly<Record<string, unknown>> | null;
}
