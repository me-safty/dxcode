...
export const UpstreamSyncSession = Schema.Struct({
  ...
  commitCount: NonNegativeInt.pipe(Schema.withDecodingDefault(Effect.succeed(0))),
  newerNightlyCount: NonNegativeInt.pipe(Schema.withDecodingDefault(Effect.succeed(0))),
  metricsHydrated: Schema.Boolean.pipe(Schema.withDecodingDefault(Effect.succeed(false))),
