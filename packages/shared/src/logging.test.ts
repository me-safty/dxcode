...
describe("RotatingFileSink", () => {
  ...
  it("keeps best-effort writes non-throwing when size recovery also fails", () => {
    const directory = makeTempDirectory();
    const logDirectory = NodePath.join(directory, "logs");
    const filePath = NodePath.join(logDirectory, "log.ndjson");
    const sink = new RotatingFileSink({ filePath, maxBytes: 100, maxFiles: 1 });
    NodeFS.rmSync(logDirectory, { recursive: true, force: true });
    NodeFS.writeFileSync(logDirectory, "occupied");
