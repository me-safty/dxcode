...
describe("readPathFromLoginShell", () => {
  it("uses a shell-agnostic printenv PATH probe", () => {
    ...
    expect(args?.[1]).toContain("__TERO_ENV_PATH_START__");
    expect(args?.[1]).toContain("__TERO_ENV_PATH_END__");
...
describe("readEnvironmentFromLoginShell", () => {
  it("extracts multiple environment variables from a login shell command", () => {
    ...
    >(() =>
      [
        ...
        "/a:/b",
        "__TERO_ENV_PATH_END__",
        "__TERO_ENV_SSH_AUTH_SOCK_START__",
        "/tmp/secretive.sock",
        "__TERO_ENV_SSH_AUTH_SOCK_END__",
      ].join("\n"),
    );
  ...
  it("preserves surrounding whitespace in captured values", () => {
    ...
    >(() =>
      ["__TERO_ENV_CUSTOM_VAR_START__", "  padded value  ", "__TERO_ENV_CUSTOM_VAR_END__"].join(
        "\n",
      ),
    );
